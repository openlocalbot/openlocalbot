...
ZIP=${1:?"Usage: $0 openlocalbot-<ver>.zip"}
FEED_URL=${2:-"https://raw.githubusercontent.com/openlocalbot/openlocalbot/main/appcast.xml"}
...
if [[ -z "$VERSION" ]]; then
  if [[ "$ZIP_NAME" =~ ^openlocalbot-([0-9]+(\.[0-9]+){1,2}([-.][^.]*)?)\.zip$ ]]; then
...
DOWNLOAD_URL_PREFIX=${SPARKLE_DOWNLOAD_URL_PREFIX:-"https://github.com/openlocalbot/openlocalbot/releases/download/v${VERSION}/"}
