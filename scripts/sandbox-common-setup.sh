...
BASE_IMAGE="${BASE_IMAGE:-openlocalbot-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-openlocalbot-sandbox-common:bookworm-slim}"
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=openlocalbot.sandbox=1)
NOTE
