...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/openlocalbot/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "openlocalbot Re-Auth" -c "After getting key, run: ssh $SERVER '~/openlocalbot/scripts/mobile-reauth.sh'" --id openlocalbot-auth
        ;;
    *)
