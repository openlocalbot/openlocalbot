...
const compilerOverride = env.openlocalbot_TS_COMPILER ?? env.CLAWDBOT_TS_COMPILER;
...
const shouldBuild = () => {
  if (env.openlocalbot_FORCE_BUILD === "1") {
    return true;
  }
...
const logRunner = (message) => {
  if (env.openlocalbot_RUNNER_LOG === "0") {
    return;
  }
  process.stderr.write(`[openlocalbot] ${message}\n`);
};

const runNode = () => {
  const nodeProcess = spawn(process.execPath, ["openlocalbot.mjs", ...args], {
    cwd,
    env,
    stdio: "inherit",
  });
