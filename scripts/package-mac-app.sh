...
# Build and bundle openlocalbot into a minimal .app we can open.
# Outputs to dist/openlocalbot.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/openlocalbot.app"
BUILD_ROOT="$ROOT_DIR/apps/macos/.build"
PRODUCT="openlocalbot"
BUNDLE_ID="${BUNDLE_ID:-ai.openlocalbot.mac.debug}"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/openlocalbot/openlocalbot/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/openlocalbot/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :openlocalbotBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :openlocalbotGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/openlocalbot"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/openlocalbot"
fi
chmod +x "$APP_ROOT/Contents/MacOS/openlocalbot"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/openlocalbot" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/openlocalbot/Resources/openlocalbot.icns" "$APP_ROOT/Contents/Resources/openlocalbot.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/openlocalbot/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "📦 Copying openlocalbotKit resources"
openlocalbotKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/openlocalbotKit_openlocalbotKit.bundle"
if [ -d "$openlocalbotKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/openlocalbotKit_openlocalbotKit.bundle"
  cp -R "$openlocalbotKIT_BUNDLE" "$APP_ROOT/Contents/Resources/openlocalbotKit_openlocalbotKit.bundle"
else
  echo "WARN: openlocalbotKit resource bundle not found at $openlocalbotKIT_BUNDLE (continuing)" >&2
fi
...
echo "⏹  Stopping any running openlocalbot"
killall -q openlocalbot 2>/dev/null || true
