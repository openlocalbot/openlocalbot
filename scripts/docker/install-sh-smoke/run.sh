...
INSTALL_URL="${openlocalbot_INSTALL_URL:-https://openlocalbot.bot/install.sh}"
SMOKE_PREVIOUS_VERSION="${openlocalbot_INSTALL_SMOKE_PREVIOUS:-}"
SKIP_PREVIOUS="${openlocalbot_INSTALL_SMOKE_SKIP_PREVIOUS:-0}"
DEFAULT_PACKAGE="openlocalbot"
PACKAGE_NAME="${openlocalbot_INSTALL_PACKAGE:-$DEFAULT_PACKAGE}"
...
if [[ "$SKIP_PREVIOUS" == "1" ]]; then
  echo "==> Skip preinstall previous (openlocalbot_INSTALL_SMOKE_SKIP_PREVIOUS=1)"
else
...
if [[ -n "${openlocalbot_INSTALL_LATEST_OUT:-}" ]]; then
  printf "%s" "$LATEST_VERSION" > "${openlocalbot_INSTALL_LATEST_OUT:-}"
fi
