#!/bin/bash
# Setup openlocalbot Auth Management System
...
echo "=== openlocalbot Auth System Setup ==="
echo ""
...
echo "  2. Subscribe to a topic (e.g., 'openlocalbot-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  openlocalbot message: Send warning via openlocalbot itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/openlocalbot-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/openlocalbot-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/openlocalbot-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now openlocalbot-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status openlocalbot-auth-monitor.timer"
echo ""
echo "Quick commands:"
