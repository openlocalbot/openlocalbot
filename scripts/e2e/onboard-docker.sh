...
IMAGE_NAME="openlocalbot-onboard-e2e"
...
docker run --rm -t "$IMAGE_NAME" bash -lc '
  ...
  cat > /tmp/openlocalbot-bin/trash <<'"'"'TRASH'"'"'
#!/usr/bin/env bash
set -euo pipefail
...
TRASH
  chmod +x /tmp/openlocalbot-bin/trash
  ...
  run_wizard_cmd() {
    ...
    input_fifo="$(mktemp -u "/tmp/openlocalbot-onboard-${case_name}.XXXXXX")"
    mkfifo "$input_fifo"
    local log_path="/tmp/openlocalbot-onboard-${case_name}.log"
  ...
  make_home() {
    mktemp -d "/tmp/openlocalbot-e2e-$1.XXXXXX"
  }
  ...
  run_case_local_basic() {
    ...
    workspace_dir="$HOME/openlocalbot"
    config_path="$HOME/.openlocalbot/openlocalbot.json"
    sessions_dir="$HOME/.openlocalbot/agents/main/sessions"
...
NODE
  ...
  run_case_reset() {
    ...
    cat > "$HOME/.openlocalbot/openlocalbot.json" <<'"'"'JSON'"'"'
{
