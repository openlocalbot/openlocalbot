...
IMAGE_NAME="openlocalbot-doctor-install-switch-e2e"
...
docker run --rm -t "$IMAGE_NAME" bash -lc '
  ...
  export PATH="/tmp/openlocalbot-bin:$PATH"
  mkdir -p /tmp/openlocalbot-bin

  cat > /tmp/openlocalbot-bin/systemctl <<"SYSTEMCTL"
#!/usr/bin/env bash
set -euo pipefail
...
SYSTEMCTL
  chmod +x /tmp/openlocalbot-bin/systemctl

  cat > /tmp/openlocalbot-bin/loginctl <<"LOGINCTL"
#!/usr/bin/env bash
set -euo pipefail
...
exit 0
LOGINCTL
  chmod +x /tmp/openlocalbot-bin/loginctl
  ...
  npm_bin="/tmp/npm-prefix/bin/openlocalbot"
  npm_entry="/tmp/npm-prefix/lib/node_modules/openlocalbot/openlocalbot.mjs"
  git_entry="/app/openlocalbot.mjs"
  ...
  run_flow() {
    ...
    echo "== Flow: $name =="
    home_dir=$(mktemp -d "/tmp/openlocalbot-switch-${name}.XXXXXX")
    export HOME="$home_dir"
    export USER="testuser"

    eval "$install_cmd"

    unit_path="$HOME/.config/systemd/user/openlocalbot-gateway.service"
    if [ ! -f "$unit_path" ]; then
