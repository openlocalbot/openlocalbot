...
IMAGE_NAME="openlocalbot-gateway-network-e2e"

PORT="18789"
TOKEN="e2e-$(date +%s)-$$"
NET_NAME="openlocalbot-net-e2e-$$"
GW_NAME="openlocalbot-gateway-e2e-$$"
...
echo "Starting gateway container..."
	docker run --rm -d \
	  --name "$GW_NAME" \
	  --network "$NET_NAME" \
	  -e "openlocalbot_GATEWAY_TOKEN=$TOKEN" \
	  -e "openlocalbot_SKIP_CHANNELS=1" \
	  -e "openlocalbot_SKIP_GMAIL_WATCHER=1" \
	  -e "openlocalbot_SKIP_CRON=1" \
	  -e "openlocalbot_SKIP_CANVAS_HOST=1" \
	  "$IMAGE_NAME" \
