...
declare const __openlocalbot_VERSION__: string | undefined;

const BUNDLED_VERSION =
  (typeof __openlocalbot_VERSION__ === "string" && __openlocalbot_VERSION__) ||
  process.env.openlocalbot_BUNDLED_VERSION ||
  "0.0.0";
...
async function main() {
  ...
  const { ensureopenlocalbotCliOnPath } = await import("../infra/path-env.js");
  ...
  process.on("uncaughtException", (error) => {
    console.error("[openlocalbot] Uncaught exception:", formatUncaughtError(error));
    process.exit(1);
  });
