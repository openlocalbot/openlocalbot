...
async function main() {
  if (hasFlag(args, "--version") || hasFlag(args, "-v")) {
    // Match `openlocalbot --version` behavior for Swift env/version checks.
  ...
  const portRaw =
    argValue(args, "--port") ??
    process.env.openlocalbot_GATEWAY_PORT ??
  ...
  const bindRaw =
    argValue(args, "--bind") ??
    process.env.openlocalbot_GATEWAY_BIND ??
...
void main().catch((err) => {
  console.error(
    "[openlocalbot] Gateway daemon failed:",
