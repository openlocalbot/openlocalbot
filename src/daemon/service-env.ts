...
export function buildServiceEnvironment(params: {
  ...
  const profile = env.openlocalbot_PROFILE;
  const resolvedLaunchdLabel =
    launchdLabel ||
    (process.platform === "darwin" ? resolveGatewayLaunchAgentLabel(profile) : undefined);
  const systemdUnit = `${resolveGatewaySystemdServiceName(profile)}.service`;
  const stateDir = env.openlocalbot_STATE_DIR;
  const configPath = env.openlocalbot_CONFIG_PATH;
  return {
    HOME: env.HOME,
    PATH: buildMinimalServicePath({ env }),
    openlocalbot_PROFILE: profile,
    openlocalbot_STATE_DIR: stateDir,
    openlocalbot_CONFIG_PATH: configPath,
    openlocalbot_GATEWAY_PORT: String(port),
    openlocalbot_GATEWAY_TOKEN: token,
    openlocalbot_LAUNCHD_LABEL: resolvedLaunchdLabel,
    openlocalbot_SYSTEMD_UNIT: systemdUnit,
    openlocalbot_SERVICE_MARKER: GATEWAY_SERVICE_MARKER,
    openlocalbot_SERVICE_KIND: GATEWAY_SERVICE_KIND,
...
export function buildNodeServiceEnvironment(params: {
  ...
  return {
    ...
    openlocalbot_STATE_DIR: stateDir,
    openlocalbot_CONFIG_PATH: configPath,
    openlocalbot_LAUNCHD_LABEL: resolveNodeLaunchAgentLabel(),
    openlocalbot_SYSTEMD_UNIT: resolveNodeSystemdServiceName(),
    openlocalbot_WINDOWS_TASK_NAME: resolveNodeWindowsTaskName(),
    openlocalbot_TASK_SCRIPT_NAME: NODE_WINDOWS_TASK_SCRIPT_NAME,
    openlocalbot_LOG_PREFIX: "node",
    openlocalbot_SERVICE_MARKER: NODE_SERVICE_MARKER,
    openlocalbot_SERVICE_KIND: NODE_SERVICE_KIND,
    openlocalbot_SERVICE_VERSION: VERSION,
  };
}
