...
function setSlackDmPolicy(cfg: openlocalbotConfig, dmPolicy: DmPolicy) {
  const allowFrom =
...
function buildSlackManifest(botName: string) {
  const safeName = botName.trim() || "openlocalbot";
  const manifest = {
    display_information: {
      name: safeName,
      description: `${safeName} connector for openlocalbot`,
    },
    features: {
      bot_user: {
...
function setSlackAllowFrom(cfg: openlocalbotConfig, allowFrom: string[]): openlocalbotConfig {
  return {
    ...cfg,
    channels: {
      ...cfg.channels,
