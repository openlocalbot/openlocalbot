...
const execHostEnforced = process.env.openlocalbot_NODE_EXEC_HOST?.trim().toLowerCase() === "app";
const execHostFallbackAllowed =
  process.env.openlocalbot_NODE_EXEC_FALLBACK?.trim().toLowerCase() !== "0";
...
function ensureNodePathEnv(): string {
  ensureopenlocalbotCliOnPath({ pathEnv: process.env.PATH ?? "" });
