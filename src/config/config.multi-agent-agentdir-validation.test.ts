...
describe("multi-agent agentDir validation", () => {
  it("rejects shared agents.list agentDir", async () => {
    ...
    const shared = path.join(tmpdir(), "openlocalbot-shared-agentdir");
        ...
        JSON.stringify(
          {
            agents: {
              list: [
                { id: "a", agentDir: "~/.openlocalbot/agents/shared/agent" },
                { id: "b", agentDir: "~/.openlocalbot/agents/shared/agent" },
              ],
            },
