...
const ETC_openlocalbot_DIR = path.join(ROOT_DIR, "etc", "openlocalbot");
const SHARED_DIR = path.join(ROOT_DIR, "shared");

const DEFAULT_BASE_PATH = path.join(CONFIG_DIR, "openlocalbot.json");

function configPath(...parts: string[]) {
  return path.join(CONFIG_DIR, ...parts);
}

function etcopenlocalbotPath(...parts: string[]) {
  return path.join(ETC_openlocalbot_DIR, ...parts);
}
...
describe("resolveConfigIncludes", () => {
  ...
  it("resolves absolute path $include", () => {
    const absolute = etcopenlocalbotPath("agents.json");
  ...
  it("resolves parent directory references", () => {
    ...
    expect(resolve(obj, files, configPath("sub", "openlocalbot.json"))).toEqual({
      shared: true,
    });
  });
});
