...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".openlocalbot-profile-decorated");
}
 ...
 */
export function decorateopenlocalbotProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_openlocalbot_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_openlocalbot_BROWSER_COLOR).toUpperCase();
