...
import {
  ...
  DEFAULT_openlocalbot_BROWSER_ENABLED,
...
function normalizeHexColor(raw: string | undefined) {
  ...
  if (!value) {
    return DEFAULT_openlocalbot_BROWSER_COLOR;
  }
...
/**
 * Ensure the default "openlocalbot" profile exists in the profiles map.
...
function ensureDefaultProfile(
  ...
  if (!result[DEFAULT_openlocalbot_BROWSER_PROFILE_NAME]) {
    result[DEFAULT_openlocalbot_BROWSER_PROFILE_NAME] = {
 ...
 *
 * Note: this is an openlocalbot browser profile (routing config), not a Chrome user profile.
...
function ensureDefaultChromeExtensionProfile(
  ...
  // (legacy single-profile configs may use controlPort+1 for openlocalbot/openlocalbot CDP).
...
export function resolveProfile(
  ...
  let cdpUrl = "";
  const driver = profile.driver === "extension" ? "extension" : "openlocalbot";
