...
vi.mock("./chrome.js", () => ({
  resolveopenlocalbotUserDataDir: vi.fn(() => "/tmp/openlocalbot-test/openlocalbot/user-data"),
}));

import { loadConfig, writeConfigFile } from "../config/config.js";
import { resolveopenlocalbotUserDataDir } from "./chrome.js";
...
describe("BrowserProfilesService", () => {
  ...
  it("deletes local profiles and moves data to Trash", async () => {
    ...
    const tempDir = fs.mkdtempSync(path.join("/tmp", "openlocalbot-profile-"));
    const userDataDir = path.join(tempDir, "work", "user-data");
    fs.mkdirSync(path.dirname(userDataDir), { recursive: true });
    vi.mocked(resolveopenlocalbotUserDataDir).mockReturnValue(userDataDir);
