...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.openlocalbot_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*openlocalbot-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "openlocalbot";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "openlocalbot");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "openlocalbot-gw");
  ...
  records.push(`_openlocalbot-gw._tcp IN PTR ${instanceLabel}._openlocalbot-gw._tcp`);
  records.push(`${instanceLabel}._openlocalbot-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._openlocalbot-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; openlocalbot-content-hash: ${contentHash}\n${contentBody}`;
}
