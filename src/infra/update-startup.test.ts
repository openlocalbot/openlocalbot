...
describe("update-startup", () => {
  ...
  beforeEach(async () => {
    ...
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "openlocalbot-update-check-"));
  ...
  it("logs update hint for npm installs when newer tag exists", async () => {
    const { resolveopenlocalbotPackageRoot } = await import("./openlocalbot-root.js");
    ...
    vi.mocked(resolveopenlocalbotPackageRoot).mockResolvedValue("/opt/openlocalbot");
    vi.mocked(checkUpdateStatus).mockResolvedValue({
      root: "/opt/openlocalbot",
      installKind: "package",
      packageManager: "npm",
