...
export function triggeropenlocalbotRestart(): RestartAttempt {
  ...
  if (process.platform !== "darwin") {
    if (process.platform === "linux") {
      const unit = normalizeSystemdUnit(
        process.env.openlocalbot_SYSTEMD_UNIT,
        process.env.openlocalbot_PROFILE,
      );
  ...
  const label =
    process.env.openlocalbot_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.openlocalbot_PROFILE);
