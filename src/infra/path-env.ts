...
type EnsureopenlocalbotPathOpts = {
  execPath?: string;
  cwd?: string;
  homeDir?: string;
...
function candidateBinDirs(opts: EnsureopenlocalbotPathOpts): string[] {
  ...
  // Bundled macOS app: `openlocalbot` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCli = path.join(execDir, "openlocalbot");
  ...
  // Project-local installs (best effort): if a `node_modules/.bin/openlocalbot` exists near cwd,
  // include it. This helps when running under launchd or other minimal PATH environments.
  const localBinDir = path.join(cwd, "node_modules", ".bin");
  if (isExecutable(path.join(localBinDir, "openlocalbot"))) {
...
/**
 * Best-effort PATH bootstrap so skills that require the `openlocalbot` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureopenlocalbotCliOnPath(opts: EnsureopenlocalbotPathOpts = {}) {
  if (isTruthyEnvValue(process.env.openlocalbot_PATH_BOOTSTRAPPED)) {
    return;
  }
  process.env.openlocalbot_PATH_BOOTSTRAPPED = "1";
