...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(path.join(os.tmpdir(), "openlocalbot-update-preflight-"));
    ...
    const doctorStep = await runStep(
      step(
        "openlocalbot doctor",
        managerScriptArgs(manager, "openlocalbot", ["doctor", "--non-interactive"]),
        gitRoot,
        { openlocalbot_UPDATE_IN_PROGRESS: "1" },
      ),
    );
