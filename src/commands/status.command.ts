...
export async function statusCommand(
  ...
  runtime.log(theme.heading("openlocalbot status"));
  runtime.log("");
  ...
  runtime.log(theme.muted(`Full report: ${formatCliCommand("openlocalbot security audit")}`));
  runtime.log(theme.muted(`Deep probe: ${formatCliCommand("openlocalbot security audit --deep")}`));
  ...
  runtime.log("");
  runtime.log("FAQ: https://docs.openlocalbot.ai/faq");
  runtime.log("Troubleshooting: https://docs.openlocalbot.ai/troubleshooting");
  runtime.log("");
  ...
  runtime.log(`  Need to share?      ${formatCliCommand("openlocalbot status --all")}`);
  runtime.log(`  Need to debug live? ${formatCliCommand("openlocalbot logs --follow")}`);
  if (gatewayReachable) {
    runtime.log(`  Need to test channels? ${formatCliCommand("openlocalbot status --deep")}`);
  } else {
    runtime.log(`  Fix reachability first: ${formatCliCommand("openlocalbot gateway probe")}`);
  }
}
