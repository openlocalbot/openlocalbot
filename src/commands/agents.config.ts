...
export function listAgentEntries(cfg: openlocalbotConfig): AgentEntry[] {
...
function resolveAgentName(cfg: openlocalbotConfig, agentId: string) {
...
function resolveAgentModel(cfg: openlocalbotConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: openlocalbotConfig): AgentSummary[] {
