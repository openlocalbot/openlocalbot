...
import type { OpenLocalBotConfig, GatewayBindMode } from "../config/config.js";
...
export async function noteSecurityWarnings(cfg: OpenLocalBotConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("openlocalbot security audit --deep")}`;
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("openlocalbot configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("openlocalbot config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("openlocalbot doctor --fix")} to generate a token`,
      ...
      warnings.push(
        ...
        `  Fix: ${formatCliCommand("openlocalbot config set gateway.bind loopback")}`,
        ...authFixLines,
      );
    } else {
