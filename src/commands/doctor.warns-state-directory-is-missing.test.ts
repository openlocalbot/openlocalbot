...
beforeEach(() => {
  ...
  resolveopenlocalbotPackageRoot.mockReset().mockResolvedValue(null);
  ...
  setStdinTty(true);
  originalStateDir = process.env.openlocalbot_STATE_DIR;
  originalUpdateInProgress = process.env.openlocalbot_UPDATE_IN_PROGRESS;
  process.env.openlocalbot_UPDATE_IN_PROGRESS = "1";
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "openlocalbot-doctor-state-"));
...
afterEach(() => {
  ...
  if (originalStateDir === undefined) {
    ...
    process.env.openlocalbot_STATE_DIR = originalStateDir;
  }
  if (originalUpdateInProgress === undefined) {
    delete process.env.openlocalbot_UPDATE_IN_PROGRESS;
  } else {
    process.env.openlocalbot_UPDATE_IN_PROGRESS = originalUpdateInProgress;
  }
  if (tempStateDir) {
...
const resolveopenlocalbotPackageRoot = vi.fn().mockResolvedValue(null);
...
vi.mock("../plugins/loader.js", () => ({
  loadopenlocalbotPlugins: () => ({ plugins: [], diagnostics: [] }),
}));
...
vi.mock("../config/config.js", async (importOriginal) => {
  ...
  return {
    ...actual,
    CONFIG_PATH: "/tmp/openlocalbot.json",
    createConfigIO,
...
vi.mock("../infra/openlocalbot-root.js", () => ({
  resolveopenlocalbotPackageRoot,
}));
...
describe("doctor command", () => {
  it("warns when the state directory is missing", async () => {
    ...
    const missingDir = fs.mkdtempSync(path.join(os.tmpdir(), "openlocalbot-missing-state-"));
    fs.rmSync(missingDir, { recursive: true, force: true });
    process.env.openlocalbot_STATE_DIR = missingDir;
    note.mockClear();
  ...
  it("skips gateway auth warning when openlocalbot_GATEWAY_TOKEN is set", async () => {
    readConfigFileSnapshot.mockResolvedValue({
      path: "/tmp/openlocalbot.json",
      exists: true,
      raw: "{}",
    ...
    process.env.openlocalbot_GATEWAY_TOKEN = "env-token-1234567890";
    note.mockClear();
