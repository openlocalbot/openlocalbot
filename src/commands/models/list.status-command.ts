...
export async function modelsStatusCommand(
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("openlocalbot models auth setup-token")}\` or \`${formatCliCommand("openlocalbot configure")}\`.`
          : `Run \`${formatCliCommand("openlocalbot configure")}\` or set an API key env var.`;
