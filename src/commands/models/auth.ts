...
import { readConfigFileSnapshot, type openlocalbotConfig } from "../../config/config.js";
...
function applyDefaultModel(cfg: openlocalbotConfig, model: string): openlocalbotConfig {
...
export async function modelsAuthLoginCommand(opts: LoginOptions, runtime: RuntimeEnv) {
  ...
  if (providers.length === 0) {
    throw new Error(
      `No provider plugins found. Install one via \`${formatCliCommand("openlocalbot plugins install")}\`.`,
    );
  }
