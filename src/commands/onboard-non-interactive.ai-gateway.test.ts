...
describe("onboard (non-interactive): Vercel AI Gateway", () => {
  it("stores the API key and configures the default model", async () => {
    ...
    const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "openlocalbot-onboard-gateway-"));
    process.env.HOME = tempHome;
    process.env.openlocalbot_STATE_DIR = tempHome;
    process.env.openlocalbot_CONFIG_PATH = path.join(tempHome, "openlocalbot.json");
    vi.resetModules();
