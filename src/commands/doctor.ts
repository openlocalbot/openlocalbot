...
function resolveMode(cfg: OpenLocalBotConfig): "local" | "remote" {
...
export async function doctorCommand(
  ...
  intro("openlocalbot doctor");

  const root = await resolveopenlocalbotPackageRoot({
  ...
  let cfg: OpenLocalBotConfig = configResult.cfg;

  const configPath = configResult.path ?? CONFIG_PATH;
  if (!cfg.gateway?.mode) {
    const lines = [
      "gateway.mode is unset; gateway start will be blocked.",
      `Fix: run ${formatCliCommand("openlocalbot configure")} and set Gateway mode (local/remote).`,
      `Or set directly: ${formatCliCommand("openlocalbot config set gateway.mode local")}`,
    ];
    if (!fs.existsSync(configPath)) {
      lines.push(`Missing config: run ${formatCliCommand("openlocalbot setup")} first.`);
    }
    note(lines.join("\n"), "Gateway");
  }
  ...
  if (shouldWriteConfig) {
    ...
    runtime.log(`Run "${formatCliCommand("openlocalbot doctor --fix")}" to apply changes.`);
  }
