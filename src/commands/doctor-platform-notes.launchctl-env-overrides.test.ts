...
describe("noteMacLaunchctlGatewayEnvOverrides", () => {
  it("prints clear unsetenv instructions for token override", async () => {
    ...
    const getenv = vi.fn(async (name: string) =>
      name === "openlocalbot_GATEWAY_TOKEN" ? "launchctl-token" : undefined,
    );
    const cfg = {
      gateway: {
        auth: {
    ...
    expect(message).toContain("openlocalbot_GATEWAY_TOKEN");
    expect(message).toContain("launchctl unsetenv openlocalbot_GATEWAY_TOKEN");
    expect(message).not.toContain("openlocalbot_GATEWAY_PASSWORD");
  });
