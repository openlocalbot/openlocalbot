...
const requireAgentDir = () => {
  const agentDir = process.env.openlocalbot_AGENT_DIR;
  if (!agentDir) {
    throw new Error("openlocalbot_AGENT_DIR not set");
  }
  return agentDir;
};
...
describe("writeOAuthCredentials", () => {
  ...
  afterEach(async () => {
    ...
    delete process.env.openlocalbot_OAUTH_DIR;
  });

  it("writes auth-profiles.json under openlocalbot_AGENT_DIR when set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "openlocalbot-oauth-"));
    process.env.openlocalbot_STATE_DIR = tempStateDir;
    process.env.openlocalbot_AGENT_DIR = path.join(tempStateDir, "agent");
    process.env.PI_CODING_AGENT_DIR = process.env.openlocalbot_AGENT_DIR;
...
describe("setMinimaxApiKey", () => {
  ...
  it("writes to openlocalbot_AGENT_DIR when set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "openlocalbot-minimax-"));
    process.env.openlocalbot_STATE_DIR = tempStateDir;
    process.env.openlocalbot_AGENT_DIR = path.join(tempStateDir, "custom-agent");
    process.env.PI_CODING_AGENT_DIR = process.env.openlocalbot_AGENT_DIR;
