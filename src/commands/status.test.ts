...
beforeAll(() => {
  previousProfile = process.env.openlocalbot_PROFILE;
  process.env.openlocalbot_PROFILE = "isolated";
});
...
afterAll(() => {
  if (previousProfile === undefined) {
    ...
    process.env.openlocalbot_PROFILE = previousProfile;
  }
});
...
vi.mock("../infra/openlocalbot-root.js", () => ({
  resolveopenlocalbotPackageRoot: vi.fn().mockResolvedValue("/tmp/openlocalbot"),
}));
...
vi.mock("../infra/update-check.js", () => ({
  checkUpdateStatus: vi.fn().mockResolvedValue({
    ...
    deps: {
      manager: "pnpm",
      status: "ok",
      lockfilePath: "/tmp/openlocalbot/pnpm-lock.yaml",
      markerPath: "/tmp/openlocalbot/node_modules/.modules.yaml",
    },
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines otherwise", async () => {
    ...
    expect(logs.some((l) => l.includes("openlocalbot status"))).toBe(true);
    ...
    expect(
      logs.some(
        (l) =>
          l.includes("openlocalbot status --all") ||
          l.includes("openlocalbot --profile isolated status --all") ||
          l.includes("openlocalbot status --all") ||
          l.includes("openlocalbot --profile isolated status --all"),
      ),
    ).toBe(true);
  ...
  it("shows gateway auth when reachable", async () => {
    ...
    process.env.openlocalbot_GATEWAY_TOKEN = "abcd1234";
    try {
