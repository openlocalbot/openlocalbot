...
import { loadopenlocalbotPlugins } from "./loader.js";

type TempPlugin = { dir: string; file: string; id: string };

const tempDirs: string[] = [];
const prevBundledDir = process.env.openlocalbot_BUNDLED_PLUGINS_DIR;
const EMPTY_PLUGIN_SCHEMA = { type: "object", additionalProperties: false, properties: {} };

function makeTempDir() {
  const dir = path.join(os.tmpdir(), `openlocalbot-plugin-${randomUUID()}`);
...
function writePlugin(params: {
  ...
  fs.writeFileSync(
    path.join(dir, "openlocalbot.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
...
afterEach(() => {
  ...
  if (prevBundledDir === undefined) {
    ...
    process.env.openlocalbot_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
});

describe("loadopenlocalbotPlugins", () => {
  it("disables bundled plugins by default", () => {
    ...
    process.env.openlocalbot_BUNDLED_PLUGINS_DIR = bundledDir;

    const registry = loadopenlocalbotPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
    ...
    const enabledRegistry = loadopenlocalbotPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
