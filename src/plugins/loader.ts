...
function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: openlocalbotPluginDefinition;
  register?: openlocalbotPluginDefinition["register"];
} {
  const resolved =
    moduleExport &&
  ...
  if (typeof resolved === "function") {
    return {
      register: resolved as openlocalbotPluginDefinition["register"],
    };
  }
  if (resolved && typeof resolved === "object") {
    const def = resolved as openlocalbotPluginDefinition;
...
export function loadopenlocalbotPlugins(options: PluginLoadOptions = {}): PluginRegistry {
  ...
  const discovery = discoveropenlocalbotPlugins({
    workspaceDir: options.workspaceDir,
  ...
  const jiti = createJiti(import.meta.url, {
    ...
    ...(pluginSdkAlias
      ? {
          alias: { "openlocalbot/plugin-sdk": pluginSdkAlias },
        }
      : {}),
  });
  ...
  for (const candidate of discovery.candidates) {
    ...
    let mod: openlocalbotPluginModule | null = null;
    try {
      mod = jiti(candidate.source) as openlocalbotPluginModule;
    } catch (err) {
