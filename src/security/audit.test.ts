...
describe("security audit", () => {
  ...
  it("treats Windows ACL-only perms as secure", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "openlocalbot-security-audit-win-"));
    const stateDir = path.join(tmp, "state");
    await fs.mkdir(stateDir, { recursive: true });
    const configPath = path.join(stateDir, "openlocalbot.json");
  ...
  it("flags Windows ACLs when Users can read the state dir", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "openlocalbot-security-audit-win-open-"));
    const stateDir = path.join(tmp, "state");
    await fs.mkdir(stateDir, { recursive: true });
    const configPath = path.join(stateDir, "openlocalbot.json");
    await fs.writeFile(configPath, "{}\n", "utf-8");
  ...
  it("flags Discord native commands without a guild user allowlist", async () => {
    const prevStateDir = process.env.openlocalbot_STATE_DIR;
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "openlocalbot-security-audit-discord-"));
    ...
    try {
      ...
      if (prevStateDir == null) {
        ...
        process.env.openlocalbot_STATE_DIR = prevStateDir;
      }
    }
  });
  ...
  it("does not flag Discord slash commands when dm.allowFrom includes a Discord snowflake id", async () => {
    ...
    const tmp = await fs.mkdtemp(
      path.join(os.tmpdir(), "openlocalbot-security-audit-discord-allowfrom-snowflake-"),
    );
    process.env.openlocalbot_STATE_DIR = tmp;
    await fs.mkdir(path.join(tmp, "credentials"), { recursive: true, mode: 0o700 });
    try {
  ...
  it("flags Discord slash commands when access-group enforcement is disabled and no users allowlist exists", async () => {
    const prevStateDir = process.env.openlocalbot_STATE_DIR;
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "openlocalbot-security-audit-discord-open-"));
    process.env.openlocalbot_STATE_DIR = tmp;
    await fs.mkdir(path.join(tmp, "credentials"), { recursive: true, mode: 0o700 });
    try {
      const cfg: openlocalbotConfig = {
  ...
  it("flags Slack slash commands without a channel users allowlist", async () => {
    const prevStateDir = process.env.openlocalbot_STATE_DIR;
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "openlocalbot-security-audit-slack-"));
    process.env.openlocalbot_STATE_DIR = tmp;
    await fs.mkdir(path.join(tmp, "credentials"), { recursive: true, mode: 0o700 });
    try {
      const cfg: openlocalbotConfig = {
  ...
  it("flags Slack slash commands when access-group enforcement is disabled", async () => {
    const prevStateDir = process.env.openlocalbot_STATE_DIR;
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "openlocalbot-security-audit-slack-open-"));
    process.env.openlocalbot_STATE_DIR = tmp;
    await fs.mkdir(path.join(tmp, "credentials"), { recursive: true, mode: 0o700 });
    try {
      const cfg: openlocalbotConfig = {
  ...
  it("flags Telegram group commands without a sender allowlist", async () => {
    const prevStateDir = process.env.openlocalbot_STATE_DIR;
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "openlocalbot-security-audit-telegram-"));
    process.env.openlocalbot_STATE_DIR = tmp;
    await fs.mkdir(path.join(tmp, "credentials"), { recursive: true, mode: 0o700 });
    try {
      const cfg: openlocalbotConfig = {
  ...
  it("warns when hooks token reuses the gateway env token", async () => {
    ...
    try {
      ...
      if (prevToken === undefined) {
        ...
        process.env.openlocalbot_GATEWAY_TOKEN = prevToken;
      }
    }
  });
  ...
  it("warns when state/config look like a synced folder", async () => {
    ...
    const res = await runSecurityAudit({
      ...
      includeFilesystem: false,
      includeChannelSecurity: false,
      stateDir: "/Users/test/Dropbox/.openlocalbot",
      configPath: "/Users/test/Dropbox/.openlocalbot/openlocalbot.json",
    });
  ...
  it("flags group/world-readable config include files", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "openlocalbot-security-audit-"));
    const stateDir = path.join(tmp, "state");
  ...
  it("flags extensions without plugins.allow", async () => {
    ...
    try {
      ...
      const res = await runSecurityAudit({
        ...
        includeChannelSecurity: false,
        stateDir,
        configPath: path.join(stateDir, "openlocalbot.json"),
      });
