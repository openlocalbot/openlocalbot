...
export function collectSyncedFolderFindings(params: {
  ...
  if (isProbablySyncedPath(params.stateDir) || isProbablySyncedPath(params.configPath)) {
    findings.push({
      ...
      remediation: `Keep openlocalbot_STATE_DIR on a local-only volume and re-run "${formatCliCommand("openlocalbot security audit --fix")}".`,
    });
  }
  return findings;
}
...
export function collectSecretsInConfigFindings(cfg: openlocalbotConfig): SecurityAuditFinding[] {
  ...
  if (password && !looksLikeEnvRef(password)) {
    findings.push({
      ...
      remediation:
        "Prefer openlocalbot_GATEWAY_PASSWORD (env) and remove gateway.auth.password from disk.",
    });
  }
