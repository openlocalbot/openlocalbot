...
function makeTempDir() {
  const dir = path.join(os.tmpdir(), `openlocalbot-hook-install-${randomUUID()}`);
...
describe("installHooksFromArchive", () => {
  it("installs hook packs from zip archives", async () => {
    ...
    zip.file(
      ...
      JSON.stringify({
        name: "@openlocalbot/zip-hooks",
        version: "0.0.1",
        openlocalbot: { hooks: ["./hooks/zip-hook"] },
      }),
    );
    zip.file(
  ...
  it("installs hook packs from tar archives", async () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        name: "@openlocalbot/tar-hooks",
        version: "0.0.1",
        openlocalbot: { hooks: ["./hooks/tar-hook"] },
      }),
      "utf-8",
    );
    fs.writeFileSync(
