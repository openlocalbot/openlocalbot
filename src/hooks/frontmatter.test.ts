...
`;
    ...
    expect(parsed.openlocalbot.emoji).toBe("💾");
    expect(parsed.openlocalbot.events).toEqual(["command:new"]);
  });
...
`;
    ...
    const parsed = JSON.parse(result.metadata);
    expect(parsed.openlocalbot.emoji).toBe("📝");
    expect(parsed.openlocalbot.events).toEqual(["command"]);
    expect(parsed.openlocalbot.requires.config).toEqual(["workspace.dir"]);
    expect(parsed.openlocalbot.install[0].kind).toBe("bundled");
  });

  it("handles single-line metadata (inline JSON)", () => {
    const content = `---
name: simple-hook
metadata: {"openlocalbot": {"events": ["test"]}}
---
`;
    const result = parseFrontmatter(content);
    expect(result.name).toBe("simple-hook");
    expect(result.metadata).toBe('{"openlocalbot": {"events": ["test"]}}');
  });
...
describe("resolveopenlocalbotMetadata", () => {
  it("extracts openlocalbot metadata from parsed frontmatter", () => {
    const frontmatter = {
      name: "test-hook",
    ...
    const result = resolveopenlocalbotMetadata(frontmatter);
  ...
  it("returns undefined when openlocalbot key is missing", () => {
    const frontmatter = {
  ...
  it("handles install specs", () => {
    const frontmatter = {
      metadata: JSON.stringify({
        openlocalbot: {
          events: ["command"],
          install: [
            { id: "bundled", kind: "bundled", label: "Bundled with openlocalbot" },
            { id: "npm", kind: "npm", package: "@openlocalbot/hook" },
          ],
        },
      }),
    };
    ...
    expect(result?.install?.[1].package).toBe("@openlocalbot/hook");
  });
...
`;
    ...
    const openlocalbot = resolveopenlocalbotMetadata(frontmatter);
    expect(openlocalbot).toBeDefined();
    expect(openlocalbot?.emoji).toBe("💾");
    expect(openlocalbot?.events).toEqual(["command:new"]);
    expect(openlocalbot?.requires?.config).toEqual(["workspace.dir"]);
    expect(openlocalbot?.install?.[0].kind).toBe("bundled");
  });
...
`;
    const frontmatter = parseFrontmatter(content);
    const openlocalbot = resolveopenlocalbotMetadata(frontmatter);
    expect(openlocalbot?.emoji).toBe("disk");
    expect(openlocalbot?.events).toEqual(["command:new"]);
  });
});
