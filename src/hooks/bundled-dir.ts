...
export function resolveBundledHooksDir(): string | undefined {
  const override = process.env.openlocalbot_BUNDLED_HOOKS_DIR?.trim();
  if (override) {
    return override;
  }
  ...
  // This path works when installed via npm: node_modules/openlocalbot/dist/hooks/bundled-dir.js
  try {
