...
This directory contains hooks that ship with openlocalbot. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
**Docs**: https://docs.openlocalbot.ai/hooks/soul-evil
...
Hooks can be configured in `~/.openlocalbot/openlocalbot.json`:
...
2. Restart gateway: `pkill -9 -f 'openlocalbot.*gateway' && pnpm openlocalbot gateway`
3. Enable the hook: `openlocalbot hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.openlocalbot.ai/hooks
