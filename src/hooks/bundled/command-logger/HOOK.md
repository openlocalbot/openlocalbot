...
homepage: https://docs.openlocalbot.ai/hooks#command-logger
metadata:
  {
    "openlocalbot":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.openlocalbot/logs/commands.log`
...
`~/.openlocalbot/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.openlocalbot/logs/commands.log ~/.openlocalbot/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/openlocalbot`:
   ```
   /home/username/.openlocalbot/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
