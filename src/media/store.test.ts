...
describe("media store", () => {
  ...
  const snapshotEnv = () => {
    for (const key of ["HOME", "USERPROFILE", "HOMEDRIVE", "HOMEPATH", "openlocalbot_STATE_DIR"]) {
  ...
  beforeAll(async () => {
    snapshotEnv();
    home = await fs.mkdtemp(path.join(os.tmpdir(), "openlocalbot-test-home-"));
    process.env.HOME = home;
    process.env.USERPROFILE = home;
    process.env.openlocalbot_STATE_DIR = path.join(home, ".openlocalbot");
    ...
    await fs.mkdir(path.join(home, ".openlocalbot"), { recursive: true });
  ...
  it("creates and returns media directory", async () => {
    await withTempStore(async (store, home) => {
      ...
      expect(path.normalize(dir)).toContain(`${path.sep}.openlocalbot${path.sep}media`);
