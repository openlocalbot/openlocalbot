...
export function resolveConfigDir(
  ...
  const override = env.openlocalbot_STATE_DIR?.trim() || env.CLAWDBOT_STATE_DIR?.trim();
  if (override) {
    return resolveUserPath(override);
  }
  const newDir = path.join(homedir(), ".openlocalbot");
  try {
...
// Configuration root; can be overridden via openlocalbot_STATE_DIR.
