...
beforeEach(async () => {
  ...
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "openlocalbot-web-home-"));
...
const makeSessionStore = async (
  ...
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "openlocalbot-session-"));
...
describe("web auto-reply", () => {
  ...
  it("supports always-on group activation with silent token and clears pending history", async () => {
    ...
    setLoadConfigMock(() => ({
      messages: {
        groupChat: { mentionPatterns: ["@openlocalbot"] },
      },
  ...
  it("ignores JID mentions in self-chat mode (group chats)", async () => {
    ...
    setLoadConfigMock(() => ({
      ...
      messages: {
        groupChat: {
          mentionPatterns: ["\\bopenlocalbot\\b"],
        },
      },
    }));
    ...
    // Text-based mentionPatterns still work (user can type "openlocalbot" explicitly).
    await capturedOnMessage?.({
      body: "openlocalbot ping",
      from: "123@g.us",
      conversationId: "123@g.us",
      chatId: "123@g.us",
      chatType: "group",
  ...
  it("emits heartbeat logs with connection metadata", async () => {
    vi.useFakeTimers();
    const logPath = `/tmp/openlocalbot-heartbeat-${crypto.randomUUID()}.log`;
  ...
  it("logs outbound replies to file", async () => {
    const logPath = `/tmp/openlocalbot-log-test-${crypto.randomUUID()}.log`;
    setLoggerOverride({ level: "trace", file: logPath });
