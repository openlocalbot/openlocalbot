...
import { __testing, createopenlocalbotCodingTools } from "./pi-tools.js";
...
describe("createopenlocalbotCodingTools", () => {
  ...
  it("keeps raw core tool schemas union-free", () => {
    const tools = createopenlocalbotTools();
  ...
  it("does not expose provider-specific message tools", () => {
    const tools = createopenlocalbotCodingTools({ messageProvider: "discord" });
  ...
  it("applies sandbox path guards to file_path alias", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "openlocalbot-sbx-"));
    const outsidePath = path.join(os.tmpdir(), "openlocalbot-outside.txt");
