...
describe("buildSandboxCreateArgs", () => {
  it("includes hardening and resource flags", () => {
    ...
    expect(args).toEqual(
      expect.arrayContaining([
        "create",
        "--name",
        "openlocalbot-sbx-test",
        "--label",
        "openlocalbot.sandbox=1",
        "--label",
        "openlocalbot.sessionKey=main",
        "--label",
        "openlocalbot.createdAtMs=1700000000000",
        "--label",
        "openlocalbot.sandboxBrowser=1",
        "--read-only",
        "--tmpfs",
        "/tmp",
        "--network",
  ...
  it("omits -v flags when binds is empty or undefined", () => {
    ...
    const args = buildSandboxCreateArgs({
      name: "openlocalbot-sbx-no-binds",
      cfg,
      scopeKey: "main",
      createdAtMs: 1700000000000,
    });
