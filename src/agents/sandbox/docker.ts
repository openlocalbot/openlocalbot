...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "openlocalbot.sandbox=1");
  args.push("--label", `openlocalbot.sessionKey=${params.scopeKey}`);
  args.push("--label", `openlocalbot.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `openlocalbot.configHash=${params.configHash}`);
  }
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  ...
  return await readLabel("openlocalbot.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`openlocalbot sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`openlocalbot sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("openlocalbot sandbox recreate --all");
}
