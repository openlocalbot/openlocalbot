...
function buildSandboxBrowserResolvedConfig(params: {
  ...
  return {
    ...
    color: DEFAULT_openlocalbot_BROWSER_COLOR,
    executablePath: undefined,
    headless: params.headless,
    noSandbox: false,
    attachOnly: true,
    defaultProfile: DEFAULT_openlocalbot_BROWSER_PROFILE_NAME,
    profiles: {
      [DEFAULT_openlocalbot_BROWSER_PROFILE_NAME]: {
...
export async function ensureSandboxBrowser(params: {
  ...
  if (!state.exists) {
    ...
    const args = buildSandboxCreateArgs({
      ...
      labels: { "openlocalbot.sandboxBrowser": "1" },
    });
    ...
    args.push("-e", `openlocalbot_BROWSER_HEADLESS=${params.cfg.browser.headless ? "1" : "0"}`);
    args.push("-e", `openlocalbot_BROWSER_ENABLE_NOVNC=${params.cfg.browser.enableNoVnc ? "1" : "0"}`);
    args.push("-e", `openlocalbot_BROWSER_CDP_PORT=${params.cfg.browser.cdpPort}`);
    args.push("-e", `openlocalbot_BROWSER_VNC_PORT=${params.cfg.browser.vncPort}`);
    args.push("-e", `openlocalbot_BROWSER_NOVNC_PORT=${params.cfg.browser.noVncPort}`);
  ...
  const existingProfile = existing
    ? resolveProfile(existing.bridge.state.resolved, DEFAULT_openlocalbot_BROWSER_PROFILE_NAME)
    : null;
  const shouldReuse =
