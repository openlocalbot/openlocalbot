...
describe("workspace path resolution", () => {
  it("reads relative paths against workspaceDir even after cwd changes", async () => {
    await withTempDir("openlocalbot-ws-", async (workspaceDir) => {
      await withTempDir("openlocalbot-cwd-", async (otherDir) => {
        ...
        try {
          const tools = createopenlocalbotCodingTools({ workspaceDir });
  ...
  it("lets exec workdir override the workspace default", async () => {
    await withTempDir("openlocalbot-ws-", async (workspaceDir) => {
      await withTempDir("openlocalbot-override-", async (overrideDir) => {
        const tools = createopenlocalbotCodingTools({ workspaceDir });
        const execTool = tools.find((tool) => tool.name === "exec");
        expect(execTool).toBeDefined();
...
describe("sandboxed workspace paths", () => {
  it("uses sandbox workspace for relative read/write/edit", async () => {
    await withTempDir("openlocalbot-sandbox-", async (sandboxDir) => {
      await withTempDir("openlocalbot-workspace-", async (workspaceDir) => {
        const sandbox = {
          enabled: true,
          ...
          workspaceAccess: "rw",
          containerName: "openlocalbot-sbx-test",
        ...
        const tools = createopenlocalbotCodingTools({ workspaceDir, sandbox });
        const readTool = tools.find((tool) => tool.name === "read");
        const writeTool = tools.find((tool) => tool.name === "write");
        const editTool = tools.find((tool) => tool.name === "edit");
