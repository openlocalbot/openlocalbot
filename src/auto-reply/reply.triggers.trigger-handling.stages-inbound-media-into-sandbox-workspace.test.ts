...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(async (home) => await fn(home), { prefix: "openlocalbot-triggers-" });
}
...
describe("stageSandboxMedia", () => {
  it("stages inbound media into the sandbox workspace", async () => {
    await withTempHome(async (home) => {
      const inboundDir = join(home, ".openlocalbot", "media", "inbound");
      ...
      await stageSandboxMedia({
        ...
        cfg: {
          agents: {
            defaults: {
              ...
              workspace: join(home, "openlocalbot"),
              sandbox: {
                mode: "non-main",
        ...
        workspaceDir: join(home, "openlocalbot"),
      });
