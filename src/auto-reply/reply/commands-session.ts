...
import { scheduleGatewaySigusr1Restart, triggeropenlocalbotRestart } from "../../infra/restart.js";
...
export const handleRestartCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  if (hasSigusr1Listener) {
    ...
    return {
      shouldContinue: false,
      reply: {
        text: "⚙️ Restarting openlocalbot in-process (SIGUSR1); back in a few seconds.",
      },
    };
  }
  const restartMethod = triggeropenlocalbotRestart();
  if (!restartMethod.ok) {
  ...
  return {
    shouldContinue: false,
    reply: {
      text: `⚙️ Restarting openlocalbot via ${restartMethod.method}; give me a few seconds to come back online.`,
    },
  };
};
