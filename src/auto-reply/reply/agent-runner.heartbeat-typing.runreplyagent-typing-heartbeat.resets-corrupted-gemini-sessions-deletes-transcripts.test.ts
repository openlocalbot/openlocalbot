...
describe("runReplyAgent typing (heartbeat)", () => {
  it("resets corrupted Gemini sessions and deletes transcripts", async () => {
    ...
    const stateDir = await fs.mkdtemp(path.join(tmpdir(), "openlocalbot-session-reset-"));
  ...
  it("keeps sessions intact on other errors", async () => {
    const prevStateDir = process.env.openlocalbot_STATE_DIR;
    const stateDir = await fs.mkdtemp(path.join(tmpdir(), "openlocalbot-session-noreset-"));
    process.env.openlocalbot_STATE_DIR = stateDir;
    try {
