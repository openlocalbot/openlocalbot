...
export function isCommandEnabled(cfg: OpenLocalBotConfig, commandKey: string): boolean {
...
function resolveDefaultCommandContext(cfg?: OpenLocalBotConfig): {
  provider: string;
  model: string;
} {
  const resolved = resolveConfiguredModelRef({
    cfg: cfg ?? ({} as OpenLocalBotConfig),
...
export function getCommandDetection(_cfg?: OpenLocalBotConfig): CommandDetection {
  const commands = getChatCommands();
...
export function maybeResolveTextAlias(raw: string, cfg?: OpenLocalBotConfig) {
