...
export async function handleToolsInvokeHttpRequest(
  ...
  const messageChannel = normalizeMessageChannel(
    getHeader(req, "x-openlocalbot-message-channel") ?? "",
  );
  const accountId = getHeader(req, "x-openlocalbot-account-id")?.trim() || undefined;
  ...
  const allTools = createopenlocalbotTools({
