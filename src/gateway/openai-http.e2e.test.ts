    ...
    try {
...

      {
        ...
        const res = await postChatCompletions(
          port,
          { model: "openlocalbot", messages: [{ role: "user", content: "hi" }] },
          { "x-openlocalbot-agent-id": "beta" },
        );
...

      {
        ...
        const res = await postChatCompletions(
          ...
          {
            "x-openlocalbot-agent-id": "beta",
            "x-openlocalbot-session-key": "agent:beta:openai:custom",
          },
        );
        expect(res.status).toBe(200);
