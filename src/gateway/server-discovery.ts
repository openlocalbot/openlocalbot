...
export function formatBonjourInstanceName(displayName: string) {
  ...
  if (/openlocalbot/i.test(trimmed)) {
    return trimmed;
  }
  return `${trimmed} (openlocalbot)`;
}

export function resolveBonjourCliPath(opts: ResolveBonjourCliPathOptions = {}): string | undefined {
  const env = opts.env ?? process.env;
  const envPath = env.openlocalbot_CLI_PATH?.trim();
  if (envPath) {
    return envPath;
  }
  ...
  const binCli = path.join(cwd, "bin", "openlocalbot");
  if (isFile(binCli)) {
    return binCli;
  }
...
export async function resolveTailnetDnsHint(opts?: {
  ...
  const env = opts?.env ?? process.env;
  const envRaw = env.openlocalbot_TAILNET_DNS?.trim();
