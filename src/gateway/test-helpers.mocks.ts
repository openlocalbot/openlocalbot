...
const testConfigRoot = {
  value: path.join(os.tmpdir(), `openlocalbot-gateway-test-${process.pid}-${crypto.randomUUID()}`),
};

export const setTestConfigRoot = (root: string) => {
  testConfigRoot.value = root;
  process.env.openlocalbot_CONFIG_PATH = path.join(root, "openlocalbot.json");
};
...
vi.mock("../config/config.js", async () => {
  ...
  const resolveConfigPath = () => path.join(testConfigRoot.value, "openlocalbot.json");
  ...
  return {
    ...
    loadConfig: () => {
      ...
      const defaults = {
        ...
        workspace: path.join(os.tmpdir(), "openlocalbot-gateway-test"),
        ...fileDefaults,
