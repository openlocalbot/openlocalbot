...
describe("gateway hot reload", () => {
  ...
  beforeEach(() => {
    prevSkipChannels = process.env.openlocalbot_SKIP_CHANNELS;
    prevSkipGmail = process.env.openlocalbot_SKIP_GMAIL_WATCHER;
    process.env.openlocalbot_SKIP_CHANNELS = "0";
  ...
  afterEach(() => {
    if (prevSkipChannels === undefined) {
      ...
      process.env.openlocalbot_SKIP_CHANNELS = prevSkipChannels;
    }
    if (prevSkipGmail === undefined) {
      delete process.env.openlocalbot_SKIP_GMAIL_WATCHER;
    } else {
      process.env.openlocalbot_SKIP_GMAIL_WATCHER = prevSkipGmail;
    }
  });
