...
export function resolveHooksConfig(cfg: openlocalbotConfig): HooksConfigResolved | null {
...
export function extractHookToken(req: IncomingMessage, url: URL): HookTokenResult {
  ...
  const headerToken =
    typeof req.headers["x-openlocalbot-token"] === "string"
      ? req.headers["x-openlocalbot-token"].trim()
      : "";
  if (headerToken) {
