...
export function resolveGatewayAuth(params: {
  ...
  const token =
    authConfig.token ?? env.openlocalbot_GATEWAY_TOKEN ?? env.CLAWDBOT_GATEWAY_TOKEN ?? undefined;
  const password =
    authConfig.password ??
    env.openlocalbot_GATEWAY_PASSWORD ??
...
export function assertGatewayAuthConfigured(auth: ResolvedGatewayAuth): void {
  if (auth.mode === "token" && !auth.token) {
    ...
    throw new Error(
      "gateway auth mode is token, but no token was configured (set gateway.auth.token or openlocalbot_GATEWAY_TOKEN)",
    );
  }
