...
async function setupGatewayTestHome() {
  ...
  previousStateDir = process.env.openlocalbot_STATE_DIR;
  previousConfigPath = process.env.openlocalbot_CONFIG_PATH;
  previousSkipBrowserControl = process.env.openlocalbot_SKIP_BROWSER_CONTROL_SERVER;
  previousSkipGmailWatcher = process.env.openlocalbot_SKIP_GMAIL_WATCHER;
  previousSkipCanvasHost = process.env.openlocalbot_SKIP_CANVAS_HOST;
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "openlocalbot-gateway-home-"));
...
async function resetGatewayTestState(options: { uniqueConfigRoot: boolean }) {
  ...
  tempConfigRoot = options.uniqueConfigRoot
    ? await fs.mkdtemp(path.join(tempHome, "openlocalbot-test-"))
    : path.join(tempHome, ".openlocalbot-test");
...
async function cleanupGatewayTestHome(options: { restoreEnv: boolean }) {
  ...
  if (options.restoreEnv) {
    ...
    if (previousConfigPath === undefined) {
      ...
      process.env.openlocalbot_CONFIG_PATH = previousConfigPath;
    }
    if (previousSkipBrowserControl === undefined) {
      delete process.env.openlocalbot_SKIP_BROWSER_CONTROL_SERVER;
    } else {
      process.env.openlocalbot_SKIP_BROWSER_CONTROL_SERVER = previousSkipBrowserControl;
    }
    if (previousSkipGmailWatcher === undefined) {
      delete process.env.openlocalbot_SKIP_GMAIL_WATCHER;
    } else {
      process.env.openlocalbot_SKIP_GMAIL_WATCHER = previousSkipGmailWatcher;
    }
    if (previousSkipCanvasHost === undefined) {
      delete process.env.openlocalbot_SKIP_CANVAS_HOST;
    } else {
      process.env.openlocalbot_SKIP_CANVAS_HOST = previousSkipCanvasHost;
    }
  }
...
export async function startServerWithClient(token?: string, opts?: GatewayServerOptions) {
  ...
  const prev = process.env.openlocalbot_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    ...
    process.env.openlocalbot_GATEWAY_TOKEN = fallbackToken;
  }
