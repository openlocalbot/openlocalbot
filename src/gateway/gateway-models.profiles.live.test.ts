...
import { ensureopenlocalbotModelsJson } from "../agents/models-config.js";
...
const GATEWAY_LIVE = isTruthyEnvValue(process.env.openlocalbot_LIVE_GATEWAY);
const ZAI_FALLBACK = isTruthyEnvValue(process.env.openlocalbot_LIVE_GATEWAY_ZAI_FALLBACK);
const PROVIDERS = parseFilter(process.env.openlocalbot_LIVE_GATEWAY_PROVIDERS);
...
async function runGatewayModelSuite(params: GatewayModelSuiteParams) {
  const previous = {
    ...
    agentDir: process.env.openlocalbot_AGENT_DIR,
  ...
  tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "openlocalbot-live-state-"));
  ...
  process.env.openlocalbot_AGENT_DIR = tempAgentDir;
  ...
  try {
        ...
        try {
          ...
          const toolProbe = await client.request<AgentFinalPayload>(
            "agent",
            {
              ...
              message:
                "openlocalbot live tool probe (local, safe): " +
    ...
    process.env.openlocalbot_AGENT_DIR = previous.agentDir;
    process.env.PI_CODING_AGENT_DIR = previous.piAgentDir;
    process.env.openlocalbot_STATE_DIR = previous.stateDir;
  }
}
...
describeLive("gateway live (dev agent, profile keys)", () => {
  it(
    ...
    async () => {
      ...
      const rawModels = process.env.openlocalbot_LIVE_GATEWAY_MODELS?.trim();
  ...
  it("z.ai fallback handles anthropic tool history", async () => {
    ...
    const nonceA = randomUUID();
    const nonceB = randomUUID();
    const toolProbePath = path.join(workspaceDir, `.openlocalbot-live-zai-fallback.${nonceA}.txt`);
    await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);
