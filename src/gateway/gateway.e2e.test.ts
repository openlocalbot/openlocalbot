...
describe("gateway e2e", () => {
  it(
    ...
    async () => {
      ...
      const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "openlocalbot-gw-mock-home-"));
      ...
      const workspaceDir = path.join(tempHome, "openlocalbot");
      await fs.mkdir(workspaceDir, { recursive: true });

      const nonceA = randomUUID();
      const nonceB = randomUUID();
      const toolProbePath = path.join(workspaceDir, `.openlocalbot-tool-probe.${nonceA}.txt`);
      await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);

      const configDir = path.join(tempHome, ".openlocalbot");
      ...
      process.env.openlocalbot_CONFIG_PATH = configPath;
  ...
  it("runs wizard over ws and writes auth token config", { timeout: 90_000 }, async () => {
    ...
    const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "openlocalbot-wizard-home-"));
    process.env.HOME = tempHome;
