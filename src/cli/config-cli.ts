...
async function loadValidConfig() {
  ...
  defaultRuntime.error(`Run \`${formatCliCommand("openlocalbot doctor")}\` to repair, then retry.`);
...
export function registerConfigCli(program: Command) {
  const cmd = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/config", "docs.openlocalbot.ai/cli/config")}\n`,
    )
    .option(
      "--section <section>",
