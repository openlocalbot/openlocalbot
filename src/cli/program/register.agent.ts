...
${formatHelpExamples([
  ['openlocalbot agent --to +15555550123 --message "status update"', "Start a new session."],
  ['openlocalbot agent --agent ops --message "Summarize logs"', "Use a specific agent."],
  [
    'openlocalbot agent --session-id 1234 --message "Summarize inbox" --thinking medium',
    "Target a session with explicit thinking level.",
  ],
  [
    'openlocalbot agent --to +15555550123 --message "Trace logs" --verbose on --json',
    "Enable verbose logging and JSON output.",
  ],
  ['openlocalbot agent --to +15555550123 --message "Summon reply" --deliver', "Deliver reply."],
  [
    'openlocalbot agent --agent ops --message "Generate report" --deliver --reply-channel slack --reply-to "#reports"',
    "Send reply to a different channel/target.",
  ],
])}

${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.openlocalbot.ai/cli/agent")}`,
    )
    .action(async (opts) => {
  ...
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.openlocalbot.ai/cli/agents")}\n`,
    );
...
${theme.heading("Examples:")}
${formatHelpExamples([
  ['openlocalbot agents set-identity --agent main --name "openlocalbot" --emoji "🦞"', "Set name + emoji."],
  ["openlocalbot agents set-identity --agent main --avatar avatars/openlocalbot.png", "Set avatar path."],
  [
    "openlocalbot agents set-identity --workspace ~/.openlocalbot/workspace --from-identity",
    "Load from IDENTITY.md.",
  ],
  [
    "openlocalbot agents set-identity --identity-file ~/.openlocalbot/workspace/IDENTITY.md --agent main",
