...
describe("gateway SIGTERM", () => {
  ...
  it("exits 0 on SIGTERM", { timeout: 180_000 }, async () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "openlocalbot-gateway-test-"));
    const out: string[] = [];
    const err: string[] = [];
    ...
    const env = {
      ...process.env,
      openlocalbot_NO_RESPAWN: "1",
      openlocalbot_STATE_DIR: stateDir,
      openlocalbot_SKIP_CHANNELS: "1",
      openlocalbot_SKIP_GMAIL_WATCHER: "1",
      openlocalbot_SKIP_CRON: "1",
      openlocalbot_SKIP_BROWSER_CONTROL_SERVER: "1",
      openlocalbot_SKIP_CANVAS_HOST: "1",
    };
    const bootstrapPath = path.join(stateDir, "openlocalbot-entry-bootstrap.mjs");
