...
export function registerWebhooksCli(program: Command) {
  const webhooks = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/webhooks", "docs.openlocalbot.ai/cli/webhooks")}\n`,
    );
  ...
  gmail
    .command("setup")
    .description("Configure Gmail watch + Pub/Sub + openlocalbot hooks")
    ...
    .option("--hook-url <url>", "openlocalbot hook URL")
    .option("--hook-token <token>", "openlocalbot hook token")
