...
const SAFE_DAEMON_ENV_KEYS = [
  "openlocalbot_PROFILE",
  "openlocalbot_STATE_DIR",
  "openlocalbot_CONFIG_PATH",
  "openlocalbot_GATEWAY_PORT",
  "openlocalbot_NIX_MODE",
];
...
export function renderRuntimeHints(
  ...
  if (runtime.missingUnit) {
    hints.push(`Service not installed. Run: ${formatCliCommand("openlocalbot gateway install", env)}`);
    if (fileLog) {
  ...
  if (runtime.status === "stopped") {
    ...
    if (process.platform === "darwin") {
      ...
      const task = resolveGatewayWindowsTaskName(env.openlocalbot_PROFILE);
...
export function renderGatewayServiceStartHints(env: NodeJS.ProcessEnv = process.env): string[] {
  const base = [
    formatCliCommand("openlocalbot gateway install", env),
    formatCliCommand("openlocalbot gateway", env),
  ];
