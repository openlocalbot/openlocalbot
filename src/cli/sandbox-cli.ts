...
const SANDBOX_EXAMPLES = {
  main: [
    ["openlocalbot sandbox list", "List all sandbox containers."],
    ["openlocalbot sandbox list --browser", "List only browser containers."],
    ["openlocalbot sandbox recreate --all", "Recreate all containers."],
    ["openlocalbot sandbox recreate --session main", "Recreate a specific session."],
    ["openlocalbot sandbox recreate --agent mybot", "Recreate agent containers."],
    ["openlocalbot sandbox explain", "Explain effective sandbox config."],
  ],
  list: [
    ["openlocalbot sandbox list", "List all sandbox containers."],
    ["openlocalbot sandbox list --browser", "List only browser containers."],
  ...
  recreate: [
    ["openlocalbot sandbox recreate --all", "Recreate all containers."],
    ["openlocalbot sandbox recreate --session main", "Recreate a specific session."],
    ["openlocalbot sandbox recreate --agent mybot", "Recreate a specific agent (includes sub-agents)."],
    ["openlocalbot sandbox recreate --browser --all", "Recreate only browser containers."],
    ["openlocalbot sandbox recreate --all --force", "Skip confirmation."],
  ],
  explain: [
    ["openlocalbot sandbox explain", "Show effective sandbox config."],
    ["openlocalbot sandbox explain --session agent:main:main", "Explain a specific session."],
    ["openlocalbot sandbox explain --agent work", "Explain an agent sandbox."],
    ["openlocalbot sandbox explain --json", "JSON output."],
  ],
} as const;
...
export function registerSandboxCli(program: Command) {
  const sandbox = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/sandbox", "docs.openlocalbot.ai/cli/sandbox")}\n`,
    )
    .action(() => {
