...
<title>openlocalbot Canvas</title>
<style>
...
(() => {
  ...
  const hasIOS = () =>
    !!(
      ...
      window.webkit.messageHandlers.openlocalbotCanvasA2UIAction
    );
  const hasAndroid = () =>
    !!(
      (window.openlocalbotCanvasA2UIAction &&
        typeof window.openlocalbotCanvasA2UIAction.postMessage === "function")
    );
  const hasHelper = () => typeof window.openlocalbotSendUserAction === "function";
  statusEl.innerHTML =
    "Bridge: " +
  ...
  window.addEventListener("openlocalbot:a2ui-action-status", onStatus);

  function send(name, sourceComponentId) {
    if (!hasHelper()) {
      log("No action bridge found. Ensure you're viewing this on an iOS/Android openlocalbot node canvas.");
      return;
    }
    const sendUserAction =
      typeof window.openlocalbotSendUserAction === "function"
        ? window.openlocalbotSendUserAction
        : undefined;
...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.openlocalbot_SKIP_CANVAS_HOST)) {
    return true;
  }
  if (isTruthyEnvValue(process.env.openlocalbot_SKIP_CANVAS_HOST)) {
    return true;
...
function resolveDefaultCanvasRoot(): string {
  const candidates = [path.join(os.homedir(), ".openlocalbot", "canvas")];
    ...
    try {
      ...
      if (!opened) {
        if (urlPath === "/" || urlPath.endsWith("/")) {
          ...
          res.end(
            `<!doctype html><meta charset="utf-8" /><title>openlocalbot Canvas</title><pre>Missing file.\nCreate ${rootDir}/index.html</pre>`,
          );
          return true;
        }
        res.statusCode = 404;
