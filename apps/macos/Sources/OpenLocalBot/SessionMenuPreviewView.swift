...
enum SessionMenuPreviewLoader {
    private static let logger = Logger(subsystem: "ai.openlocalbot", category: "SessionPreview")
    ...
    private static func requestPreview(
        keys: [String],
        maxItems: Int) async throws -> openlocalbotSessionsPreviewPayload
    {
    ...
    private static func snapshot(
        from entry: openlocalbotSessionPreviewEntry,
    ...
    private static func cache(payload: openlocalbotSessionsPreviewPayload, maxItems: Int) async {
    ...
    private static func previewItems(
        from payload: openlocalbotChatHistoryPayload,
        maxItems: Int) -> [SessionPreviewItem]
    {
        let boundedItems = self.normalizeMaxItems(maxItems)
        let raw: [openlocalbotKit.AnyCodable] = payload.messages ?? []
    ...
    private static func decodeMessages(_ raw: [openlocalbotKit.AnyCodable]) -> [openlocalbotChatMessage] {
        raw.compactMap { item in
            guard let data = try? JSONEncoder().encode(item) else { return nil }
            return try? JSONDecoder().decode(openlocalbotChatMessage.self, from: data)
        }
    }
    ...
    private static func previewText(for message: openlocalbotChatMessage) -> String? {
    ...
    private static func isToolCall(_ message: openlocalbotChatMessage) -> Bool {
    ...
    private static func toolNames(for message: openlocalbotChatMessage) -> [String] {
        var names: [String] = []
    ...
    private static func mediaSummary(for message: openlocalbotChatMessage) -> String? {
