...
final class PeekabooBridgeHostCoordinator {
    ...
    private let logger = Logger(subsystem: "ai.openlocalbot", category: "PeekabooBridge")

    private var host: PeekabooBridgeHost?
    private var services: openlocalbotPeekabooBridgeServices?
    private static var openlocalbotSocketPath: String {
        let fileManager = FileManager.default
        let base = fileManager.urls(for: .applicationSupportDirectory, in: .userDomainMask).first
            ?? fileManager.homeDirectoryForCurrentUser.appendingPathComponent("Library/Application Support")
        let directory = base.appendingPathComponent("openlocalbot", isDirectory: true)
    ...
    private func startIfNeeded() async {
        ...
        let services = openlocalbotPeekabooBridgeServices()
        ...
        let host = PeekabooBridgeHost(
            socketPath: Self.openlocalbotSocketPath,
            server: server,
        ...
        await host.start()
        self.logger
            .info("PeekabooBridge host started at \(Self.openlocalbotSocketPath, privacy: .public)")
    }
...
@MainActor
private final class openlocalbotPeekabooBridgeServices: PeekabooBridgeServiceProviding {
    ...
    init() {
        let logging = LoggingService(subsystem: "ai.openlocalbot.peekaboo")
