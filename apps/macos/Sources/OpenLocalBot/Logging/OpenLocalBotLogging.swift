...
enum openlocalbotLogging {
    private static let labelSeparator = "::"

    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            let (subsystem, category) = Self.parseLabel(label)
            let osHandler = openlocalbotOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = openlocalbotFileLogHandler(label: label)
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        openlocalbotLogging.bootstrapIfNeeded()
        let label = openlocalbotLogging.makeLabel(subsystem: subsystem, category: category)
...
struct openlocalbotOSLogHandler: LogHandler {
...
struct openlocalbotFileLogHandler: LogHandler {
    let label: String
    var metadata: Logger.Metadata = [:]
        ...
        line: UInt)
    {
        ...
        let (subsystem, category) = openlocalbotLogging.parseLabel(self.label)
