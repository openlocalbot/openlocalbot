...
enum DebugActions {
    private static let verboseDefaultsKey = "openlocalbot.debug.verboseMain"
    private static let sessionMenuLimit = 12
    private static let onboardingSeenKey = "openlocalbot.onboardingSeen"
    ...
    static func openConfigFolder() {
        let url = openlocalbotPaths.stateDirURL
        NSWorkspace.shared.activateFileViewerSelecting([url])
    }
    ...
    static func sendTestNotification() async {
        _ = await NotificationManager().send(title: "openlocalbot", body: "Test notification", sound: nil)
    }
    ...
    private static func resolveSessionStorePath() -> String {
        ...
        let configURL = openlocalbotPaths.configURL
        guard
