...
enum LogLocator {
    private static var logDir: URL {
        if let override = ProcessInfo.processInfo.environment["openlocalbot_LOG_DIR"],
           !override.isEmpty
        {
            return URL(fileURLWithPath: override)
        }
        let preferred = URL(fileURLWithPath: "/tmp/openlocalbot")
        return preferred
    }

    private static var stdoutLog: URL {
        logDir.appendingPathComponent("openlocalbot-stdout.log")
    }

    private static var gatewayLog: URL {
        logDir.appendingPathComponent("openlocalbot-gateway.log")
    }
    ...
    /// Returns the newest log file under /tmp/openlocalbot/ (rolling or stdout), or nil if none exist.
