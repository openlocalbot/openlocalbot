...
struct openlocalbotMacCLI {
    static func main() async {
        ...
        default:
            fputs("openlocalbot-mac: unknown command\n", stderr)
            printUsage()
            exit(1)
        }
    }
...
private func printUsage() {
    ...
    Usage:
      openlocalbot-mac connect [--url <ws://host:port>] [--token <token>] [--password <password>]
      ...
      openlocalbot-mac wizard [--url <ws://host:port>] [--token <token>] [--password <password>]
                          [--mode <local|remote>] [--workspace <path>] [--json]

    Examples:
      openlocalbot-mac connect
      openlocalbot-mac connect --url ws://127.0.0.1:18789 --json
      openlocalbot-mac discover --timeout 3000 --json
      openlocalbot-mac wizard --mode local
    """)
}
