...
enum CommandResolver {
    private static let projectRootDefaultsKey = "openlocalbot.gatewayProjectRootPath"
    private static let helperName = "openlocalbot"

    static func gatewayEntrypoint(in root: URL) -> String? {
        let distEntry = root.appendingPathComponent("dist/index.js").path
        if FileManager().isReadableFile(atPath: distEntry) { return distEntry }
        let openlocalbotEntry = root.appendingPathComponent("openlocalbot.mjs").path
        if FileManager().isReadableFile(atPath: openlocalbotEntry) { return openlocalbotEntry }
        let binEntry = root.appendingPathComponent("bin/openlocalbot.js").path
    ...
    static func errorCommand(with message: String) -> [String] {
        let script = """
        cat <<'__openlocalbot_ERR__' >&2
        \(message)
        __openlocalbot_ERR__
        exit 1
        """
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/openlocalbot")
    ...
    static func preferredPaths(home: URL, current: [String], projectRoot: URL) -> [String] {
        ...
        #endif
        let openlocalbotPaths = self.openlocalbotManagedPaths(home: home)
        if !openlocalbotPaths.isEmpty {
            extras.insert(contentsOf: openlocalbotPaths, at: 1)
        }
        extras.insert(contentsOf: self.nodeManagerBinPaths(home: home), at: 1 + openlocalbotPaths.count)
        var seen = Set<String>()
        // Preserve order while stripping duplicates so PATH lookups remain deterministic.
        return (extras + current).filter { seen.insert($0).inserted }
    }

    private static func openlocalbotManagedPaths(home: URL) -> [String] {
        let bases = [
            home.appendingPathComponent(".openlocalbot"),
        ]
        var paths: [String] = []
        for base in bases {
    ...
    static func openlocalbotExecutable(searchPaths: [String]? = nil) -> String? {
        self.findExecutable(named: self.helperName, searchPaths: searchPaths)
    }

    static func projectopenlocalbotExecutable(projectRoot: URL? = nil) -> String? {
        #if DEBUG
    ...
    static func nodeCliPath() -> String? {
        ...
        let candidates = [
            root.appendingPathComponent("openlocalbot.mjs").path,
            root.appendingPathComponent("bin/openlocalbot.js").path,
        ]
    ...
    static func hasAnyopenlocalbotInvoker(searchPaths: [String]? = nil) -> Bool {
        if self.openlocalbotExecutable(searchPaths: searchPaths) != nil { return true }
    ...
    static func openlocalbotNodeCommand(
        subcommand: String,
        extraArgs: [String] = [],
        ...
        searchPaths: [String]? = nil) -> [String]
    {
        ...
        case let .success(runtime):
            let root = self.projectRoot()
            if let openlocalbotPath = self.projectopenlocalbotExecutable(projectRoot: root) {
                return [openlocalbotPath, subcommand] + extraArgs
            }
            ...
            if let pnpm = self.findExecutable(named: "pnpm", searchPaths: searchPaths) {
                ...
                return [pnpm, "--silent", "openlocalbot", subcommand] + extraArgs
            }
            if let openlocalbotPath = self.openlocalbotExecutable(searchPaths: searchPaths) {
                return [openlocalbotPath, subcommand] + extraArgs
            }

            let missingEntry = """
            openlocalbot entrypoint missing (looked for dist/index.js or openlocalbot.mjs); run pnpm build.
            """
    ...
    static func openlocalbotCommand(
        subcommand: String,
        extraArgs: [String] = [],
        defaults: UserDefaults = .standard,
        configRoot: [String: Any]? = nil,
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real openlocalbot CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/openlocalbot"
        ...
        \(cliSection)
        \(projectSection)
        if command -v openlocalbot >/dev/null 2>&1; then
          CLI="$(command -v openlocalbot)"
          openlocalbot \(quotedArgs);
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/openlocalbot.mjs" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/openlocalbot.mjs"
            node "$PRJ/openlocalbot.mjs" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/openlocalbot.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/openlocalbot.js"
            node "$PRJ/bin/openlocalbot.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent openlocalbot"
          pnpm --silent openlocalbot \(quotedArgs);
        else
          echo "openlocalbot CLI missing on remote host"; exit 127;
        fi
        """
        let options: [String] = [
            "-o", "BatchMode=yes",
    ...
    {
        let root = configRoot ?? OpenLocalBotConfigFile.loadDict()
