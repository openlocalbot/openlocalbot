...
actor MacNodeRuntime {
    ...
    func handleInvoke(_ req: BridgeInvokeRequest) async -> BridgeInvokeResponse {
        ...
        do {
            ...
            case openlocalbotCameraCommand.snap.rawValue,
                 ...
                 openlocalbotCameraCommand.list.rawValue:
                return try await self.handleCameraInvoke(req)
            case openlocalbotLocationCommand.get.rawValue:
                return try await self.handleLocationInvoke(req)
            case MacNodeScreenCommand.record.rawValue:
                return try await self.handleScreenRecordInvoke(req)
            case openlocalbotSystemCommand.run.rawValue:
                return try await self.handleSystemRun(req)
            case openlocalbotSystemCommand.which.rawValue:
                return try await self.handleSystemWhich(req)
            case openlocalbotSystemCommand.notify.rawValue:
                return try await self.handleSystemNotify(req)
            case openlocalbotSystemCommand.execApprovalsGet.rawValue:
                return try await self.handleSystemExecApprovalsGet(req)
            case openlocalbotSystemCommand.execApprovalsSet.rawValue:
    ...
    private func handleA2UIInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        ...
        case openlocalbotCanvasA2UICommand.push.rawValue,
             openlocalbotCanvasA2UICommand.pushJSONL.rawValue:
    ...
    private func handleA2UIPush(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let command = req.command
        let messages: [openlocalbotKit.AnyCodable]
    ...
    private func resolveA2UIHostUrl() async -> String? {
        ...
        return baseUrl.appendingPathComponent("__openlocalbot__/a2ui/").absoluteString + "?platform=macos"
    }
    ...
    private func handleSystemRun(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let params = try Self.decodeParams(openlocalbotSystemRunParams.self, from: req.paramsJSON)
    ...
    private func handleSystemWhich(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let params = try Self.decodeParams(openlocalbotSystemWhichParams.self, from: req.paramsJSON)
    ...
    private func resolveSystemRunApproval(
        ...
        params: openlocalbotSystemRunParams,
    ...
    private func handleSystemNotify(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let params = try Self.decodeParams(openlocalbotSystemNotifyParams.self, from: req.paramsJSON)
...
extension MacNodeRuntime {
    ...
    private nonisolated static func locationMode() -> openlocalbotLocationMode {
        ...
        message: String) -> BridgeInvokeResponse
    {
        BridgeInvokeResponse(
            id: req.id,
            ok: false,
            error: openlocalbotNodeError(code: code, message: message))
    }

    private static func encodeCanvasSnapshot(
        image: NSImage,
        format: openlocalbotCanvasSnapshotFormat,
        maxWidth: Int?,
