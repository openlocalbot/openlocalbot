...
struct ControlAgentEvent: Codable, Sendable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: openlocalbotProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    private let logger = Logger(subsystem: "ai.openlocalbot", category: "control")
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: openlocalbotKit.AnyCodable]()) {
                $0[$1.key] = openlocalbotKit.AnyCodable($1.value.base)
            }
    ...
    private static func bridgeToProtocolArgs(
        _ value: openlocalbotProtocol.AnyCodable?) -> [String: openlocalbotProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: openlocalbotProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: openlocalbotKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: openlocalbotProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
...
extension Notification.Name {
    static let controlHeartbeat = Notification.Name("openlocalbot.control.heartbeat")
    static let controlAgentEvent = Notification.Name("openlocalbot.control.agent")
}
