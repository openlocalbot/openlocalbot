...
private let webChatSwiftLogger = Logger(subsystem: "ai.openlocalbot", category: "WebChatSwiftUI")
...
struct MacGatewayChatTransport: openlocalbotChatTransport, Sendable {
    ...
    func listSessions(limit: Int?) async throws -> openlocalbotChatSessionsListResponse {
        ...
        let data = try await GatewayConnection.shared.request(
            ...
            params: params,
            timeoutMs: 15000)
        return try JSONDecoder().decode(openlocalbotChatSessionsListResponse.self, from: data)
    }
    ...
    static func mapPushToTransportEvent(_ push: GatewayPush) -> openlocalbotChatTransportEvent? {
        switch push {
        case let .snapshot(hello):
            let ok = (try? JSONDecoder().decode(
                openlocalbotGatewayHealthOK.self,
        ...
        case let .event(evt):
            ...
            case "chat":
                ...
                guard let chat = try? JSONDecoder().decode(
                    openlocalbotChatEventPayload.self,
            ...
            case "agent":
                ...
                guard let agent = try? JSONDecoder().decode(
                    openlocalbotAgentEventPayload.self,
                    from: JSONEncoder().encode(payload))
                else {
                    return nil
                }
...
final class WebChatSwiftUIWindowController {
    ...
    private let hosting: NSHostingController<openlocalbotChatView>
    ...
    init(sessionKey: String, presentation: WebChatPresentation, transport: any openlocalbotChatTransport) {
        self.sessionKey = sessionKey
        self.presentation = presentation
        let vm = openlocalbotChatViewModel(sessionKey: sessionKey, transport: transport)
        let accent = Self.color(fromHex: AppStateStore.shared.seamColorHex)
        self.hosting = NSHostingController(rootView: openlocalbotChatView(
            viewModel: vm,
        ...
        contentViewController: NSViewController) -> NSWindow
    {
        ...
        case .window:
            let window = NSWindow(
                ...
                backing: .buffered,
                defer: false)
            window.title = "openlocalbot Chat"
    ...
    private static func makeContentController(
        for presentation: WebChatPresentation,
        hosting: NSHostingController<openlocalbotChatView>) -> NSViewController
    {
