...
enum AnthropicAuthMode: Equatable {
    ...
    var shortLabel: String {
        switch self {
        case .oauthFile: "OAuth (openlocalbot token file)"
...
enum AnthropicAuthResolver {
    static func resolve(
        ...
        oauthStatus: openlocalbotOAuthStore.AnthropicOAuthStatus = openlocalbotOAuthStore
...
enum AnthropicOAuth {
    private static let logger = Logger(subsystem: "ai.openlocalbot", category: "anthropic-oauth")
...
enum openlocalbotOAuthStore {
    static let oauthFilename = "oauth.json"
    private static let providerKey = "anthropic"
    private static let openlocalbotOAuthDirEnv = "openlocalbot_OAUTH_DIR"
    ...
    enum AnthropicOAuthStatus: Equatable {
        ...
        var shortDescription: String {
            switch self {
            case .missingFile: "openlocalbot OAuth token file not found"
            case .unreadableFile: "openlocalbot OAuth token file not readable"
            case .invalidJSON: "openlocalbot OAuth token file invalid"
            case .missingProviderEntry: "No Anthropic entry in openlocalbot OAuth token file"
            case .missingTokens: "Anthropic entry missing tokens"
            case .connected: "openlocalbot OAuth credentials found"
            }
        }
    }

    static func oauthDir() -> URL {
        if let override = ProcessInfo.processInfo.environment[self.openlocalbotOAuthDirEnv]?
