...
@Suite struct InstancesStoreTests {
    ...
    func presenceEventPayloadDecodesViaJSONEncoder() {
        ...
        let entry: [String: openlocalbotProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: openlocalbotProtocol.AnyCodable] = [
            "presence": .init([openlocalbotProtocol.AnyCodable(entry)]),
        ]
        let payload = openlocalbotProtocol.AnyCodable(payloadMap)
