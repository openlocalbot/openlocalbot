...
@Suite struct MacGatewayChatTransportMappingTests {
    @Test func snapshotMapsToHealth() {
        let snapshot = Snapshot(
            presence: [],
            health: openlocalbotProtocol.AnyCodable(["ok": openlocalbotProtocol.AnyCodable(false)]),
    ...
    @Test func healthEventMapsToHealth() {
        let frame = EventFrame(
            type: "event",
            event: "health",
            payload: openlocalbotProtocol.AnyCodable(["ok": openlocalbotProtocol.AnyCodable(true)]),
            seq: 1,
            stateversion: nil)
    ...
    @Test func chatEventMapsToChat() {
        let payload = openlocalbotProtocol.AnyCodable([
            "runId": openlocalbotProtocol.AnyCodable("run-1"),
            "sessionKey": openlocalbotProtocol.AnyCodable("main"),
            "state": openlocalbotProtocol.AnyCodable("final"),
        ])
    ...
    @Test func unknownEventMapsToNil() {
        let frame = EventFrame(
            type: "event",
            event: "unknown",
            payload: openlocalbotProtocol.AnyCodable(["a": openlocalbotProtocol.AnyCodable(1)]),
            seq: 1,
            stateversion: nil)
        let mapped = MacGatewayChatTransport.mapPushToTransportEvent(.event(frame))
        #expect(mapped == nil)
