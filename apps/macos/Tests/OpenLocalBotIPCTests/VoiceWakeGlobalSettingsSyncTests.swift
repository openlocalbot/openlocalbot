...
@Suite(.serialized) struct VoiceWakeGlobalSettingsSyncTests {
    @Test func appliesVoiceWakeChangedEventToAppState() async {
        ...
        let payload = openlocalbotProtocol.AnyCodable(["triggers": ["openlocalbot", "computer"]])
        let evt = EventFrame(
            type: "event",
        ...
        #expect(updated == ["openlocalbot", "computer"])
    ...
    @Test func ignoresVoiceWakeChangedEventWithInvalidPayload() async {
        ...
        let payload = openlocalbotProtocol.AnyCodable(["unexpected": 123])
        let evt = EventFrame(
            type: "event",
            event: "voicewake.changed",
            payload: payload,
