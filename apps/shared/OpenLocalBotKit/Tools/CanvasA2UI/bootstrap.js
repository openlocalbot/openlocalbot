...
import "@a2ui/lit/ui";
import { themeContext } from "@openlocalbot/a2ui-theme-context";
...
class openlocalbotA2UIHost extends LitElement {
  static properties = {
    surfaces: { state: true },
  ...
  themeProvider = new ContextProvider(this, {
    ...
    initialValue: openlocalbotTheme,
  });
  ...
  static styles = css`
    :host {
      ...
      padding:
        var(--openlocalbot-a2ui-inset-top, 0px)
        var(--openlocalbot-a2ui-inset-right, 0px)
        var(--openlocalbot-a2ui-inset-bottom, 0px)
        var(--openlocalbot-a2ui-inset-left, 0px);
    }
    ...
    #surfaces {
      ...
      height: 100%;
      overflow: auto;
      padding-bottom: var(--openlocalbot-a2ui-scroll-pad-bottom, 0px);
    }
    ...
    .status {
      ...
      top: var(--openlocalbot-a2ui-status-top, 12px);
      display: inline-flex;
      align-items: center;
      gap: 8px;
      padding: 8px 10px;
    ...
    .toast {
      ...
      left: 50%;
      transform: translateX(-50%);
      bottom: var(--openlocalbot-a2ui-toast-bottom, 12px);
      display: inline-flex;
      align-items: center;
      gap: 8px;
      padding: 8px 10px;
    ...
    .empty {
      ...
      left: 50%;
      transform: translateX(-50%);
      top: var(--openlocalbot-a2ui-empty-top, var(--openlocalbot-a2ui-status-top, 12px));
      text-align: center;
      opacity: 0.8;
      padding: 10px 12px;
      pointer-events: none;
  ...
  connectedCallback() {
    ...
    globalThis.openlocalbotA2UI = api;
    this.addEventListener("a2uiaction", (evt) => this.#handleA2UIAction(evt));
    this.#statusListener = (evt) => this.#handleActionStatus(evt);
    for (const eventName of ["openlocalbot:a2ui-action-status"]) {
  ...
  #handleA2UIAction(evt) {
    ...
    globalThis.__openlocalbotLastA2UIAction = userAction;

    const handler =
      globalThis.webkit?.messageHandlers?.openlocalbotCanvasA2UIAction ??
      globalThis.openlocalbotCanvasA2UIAction;
    if (handler?.postMessage) {
      try {
        // WebKit message handlers support structured objects; Android's JS interface expects strings.
        if (handler === globalThis.openlocalbotCanvasA2UIAction) {
...
if (!customElements.get("openlocalbot-a2ui-host")) {
  customElements.define("openlocalbot-a2ui-host", openlocalbotA2UIHost);
}
