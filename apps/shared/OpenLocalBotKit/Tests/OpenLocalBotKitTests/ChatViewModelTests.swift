...
import Foundation
import Testing
@testable import openlocalbotChatUI
...
private final class TestChatTransport: @unchecked Sendable, openlocalbotChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [openlocalbotChatHistoryPayload]
    private let sessionsResponses: [openlocalbotChatSessionsListResponse]

    private let stream: AsyncStream<openlocalbotChatTransportEvent>
    private let continuation: AsyncStream<openlocalbotChatTransportEvent>.Continuation

    init(
        historyResponses: [openlocalbotChatHistoryPayload],
        sessionsResponses: [openlocalbotChatSessionsListResponse] = [])
    {
        self.historyResponses = historyResponses
        self.sessionsResponses = sessionsResponses
        var cont: AsyncStream<openlocalbotChatTransportEvent>.Continuation!
    ...
    func requestHistory(sessionKey: String) async throws -> openlocalbotChatHistoryPayload {
        ...
        return self.historyResponses.last ?? openlocalbotChatHistoryPayload(
        ...
        attachments _: [openlocalbotChatAttachmentPayload]) async throws -> openlocalbotChatSendResponse
    {
        ...
        return openlocalbotChatSendResponse(runId: idempotencyKey, status: "ok")
    }
    ...
    func listSessions(limit _: Int?) async throws -> openlocalbotChatSessionsListResponse {
        ...
        return self.sessionsResponses.last ?? openlocalbotChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    func emit(_ evt: openlocalbotChatTransportEvent) {
...
@Suite struct ChatViewModelTests {
    @Test func streamsAssistantAndClearsOnFinal() async throws {
        ...
        let history1 = openlocalbotChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
        let history2 = openlocalbotChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [
                AnyCodable([
        ...
        let vm = await MainActor.run { openlocalbotChatViewModel(sessionKey: "main", transport: transport) }
    ...
    @Test func clearsStreamingOnExternalFinalEvent() async throws {
        let sessionId = "sess-main"
        let history = openlocalbotChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
    ...
    @Test func sessionChoicesPreferMainAndRecent() async throws {
        ...
        let history = openlocalbotChatHistoryPayload(
            ...
            messages: [],
            thinkingLevel: "off")
        let sessions = openlocalbotChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 4,
            defaults: nil,
    ...
    @Test func sessionChoicesIncludeCurrentWhenMissing() async throws {
        ...
        let transport = TestChatTransport(
            historyResponses: [history],
            sessionsResponses: [sessions])
        let vm = await MainActor.run { openlocalbotChatViewModel(sessionKey: "custom", transport: transport) }
        await MainActor.run { vm.load() }
        try await waitUntil("sessions loaded") { await MainActor.run { !vm.sessions.isEmpty } }
