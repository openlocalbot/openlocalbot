...
enum openlocalbotChatTheme {
    ...
    static let assistantBubbleDynamicNSColor = NSColor(
        name: NSColor.Name("openlocalbotChatTheme.assistantBubble"),
        dynamicProvider: resolvedAssistantBubbleColor(for:))

    static let onboardingAssistantBubbleDynamicNSColor = NSColor(
        name: NSColor.Name("openlocalbotChatTheme.onboardingAssistantBubble"),
...
enum openlocalbotPlatformImageFactory {
    static func image(_ image: openlocalbotPlatformImage) -> Image {
        #if os(macOS)
        Image(nsImage: image)
        #else
        Image(uiImage: image)
