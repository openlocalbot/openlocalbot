...
struct IOSGatewayChatTransport: openlocalbotChatTransport, Sendable {
    ...
    func listSessions(limit: Int?) async throws -> openlocalbotChatSessionsListResponse {
        ...
        return try JSONDecoder().decode(openlocalbotChatSessionsListResponse.self, from: res)
    }
    ...
    func requestHistory(sessionKey: String) async throws -> openlocalbotChatHistoryPayload {
        ...
        return try JSONDecoder().decode(openlocalbotChatHistoryPayload.self, from: res)
    }
        ...
        attachments: [openlocalbotChatAttachmentPayload]) async throws -> openlocalbotChatSendResponse
    {
        struct Params: Codable {
            ...
            var message: String
            var thinking: String
            var attachments: [openlocalbotChatAttachmentPayload]?
            var timeoutMs: Int
        ...
        return try JSONDecoder().decode(openlocalbotChatSendResponse.self, from: res)
    }

    func requestHealth(timeoutMs: Int) async throws -> Bool {
        let seconds = max(1, Int(ceil(Double(timeoutMs) / 1000.0)))
        let res = try await self.gateway.request(method: "health", paramsJSON: nil, timeoutSeconds: seconds)
        return (try? JSONDecoder().decode(openlocalbotGatewayHealthOK.self, from: res))?.ok ?? true
    }
                ...
                for await evt in stream {
                    ...
                    case "health":
                        ...
                        let ok = (try? GatewayPayloadDecoding.decode(
                            payload,
                            as: openlocalbotGatewayHealthOK.self))?.ok ?? true
                        continuation.yield(.health(ok: ok))
                    case "chat":
                        guard let payload = evt.payload else { break }
                        if let chatPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: openlocalbotChatEventPayload.self)
                        {
                    ...
                    case "agent":
                        ...
                        if let agentPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: openlocalbotAgentEventPayload.self)
                        {
