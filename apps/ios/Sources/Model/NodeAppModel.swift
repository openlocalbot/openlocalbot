...
final class NodeAppModel {
    ...
    private func handleCanvasA2UIAction(body: [String: Any]) async {
        ...
        guard let name = openlocalbotCanvasA2UIAction.extractActionName(userAction) else { return }
        let actionId: String = {
        ...
        let contextJSON = openlocalbotCanvasA2UIAction.compactJSON(userAction["context"])
        let sessionKey = self.mainSessionKey

        let messageContext = openlocalbotCanvasA2UIAction.AgentMessageContext(
            actionName: name,
            session: .init(key: sessionKey, surfaceId: surfaceId),
            component: .init(id: sourceComponentId, host: host, instanceId: instanceId),
            contextJSON: contextJSON)
        let message = openlocalbotCanvasA2UIAction.formatAgentMessage(messageContext)
        ...
        let js = openlocalbotCanvasA2UIAction.jsDispatchA2UIActionStatus(actionId: actionId, ok: ok, error: errorText)
        do {
    ...
    private func resolveA2UIHostURL() async -> String? {
        ...
        return base.appendingPathComponent("__openlocalbot__/a2ui/").absoluteString + "?platform=ios"
    }
    ...
    func requestLocationPermissions(mode: openlocalbotLocationMode) async -> Bool {
    ...
    private func sendAgentRequest(link: AgentDeepLink) async throws {
        ...
        // (Key-based unattended auth is handled on macOS for openlocalbot:// links.)
    ...
    private func handleInvoke(_ req: BridgeInvokeRequest) async -> BridgeInvokeResponse {
        ...
        do {
            switch command {
            case openlocalbotLocationCommand.get.rawValue:
                return try await self.handleLocationInvoke(req)
            case openlocalbotCanvasCommand.present.rawValue,
            ...
            case openlocalbotCanvasA2UICommand.reset.rawValue,
            ...
            case openlocalbotCameraCommand.list.rawValue,
                 openlocalbotCameraCommand.snap.rawValue,
                 openlocalbotCameraCommand.clip.rawValue:
                return try await self.handleCameraInvoke(req)
            case openlocalbotScreenCommand.record.rawValue:
                return try await self.handleScreenRecordInvoke(req)
            default:
                return BridgeInvokeResponse(
                    id: req.id,
                    ok: false,
                    error: openlocalbotNodeError(code: .invalidRequest, message: "INVALID_REQUEST: unknown command"))
            }
        } catch {
            ...
            return BridgeInvokeResponse(
                id: req.id,
                ok: false,
                error: openlocalbotNodeError(code: .unavailable, message: error.localizedDescription))
        }
    }
    ...
    private func handleLocationInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        ...
        let params = (try? Self.decodeParams(openlocalbotLocationGetParams.self, from: req.paramsJSON)) ??
            openlocalbotLocationGetParams()
        ...
        let payload = openlocalbotLocationPayload(
    ...
    private func handleCanvasInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case openlocalbotCanvasCommand.present.rawValue:
            let params = (try? Self.decodeParams(openlocalbotCanvasPresentParams.self, from: req.paramsJSON)) ??
                openlocalbotCanvasPresentParams()
        ...
        case openlocalbotCanvasCommand.hide.rawValue:
            return BridgeInvokeResponse(id: req.id, ok: true)
        case openlocalbotCanvasCommand.navigate.rawValue:
            let params = try Self.decodeParams(openlocalbotCanvasNavigateParams.self, from: req.paramsJSON)
            self.screen.navigate(to: params.url)
            return BridgeInvokeResponse(id: req.id, ok: true)
        case openlocalbotCanvasCommand.evalJS.rawValue:
            let params = try Self.decodeParams(openlocalbotCanvasEvalParams.self, from: req.paramsJSON)
            let result = try await self.screen.eval(javaScript: params.javaScript)
            let payload = try Self.encodePayload(["result": result])
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case openlocalbotCanvasCommand.snapshot.rawValue:
            let params = try? Self.decodeParams(openlocalbotCanvasSnapshotParams.self, from: req.paramsJSON)
    ...
    private func handleCanvasA2UIInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        ...
        case openlocalbotCanvasA2UICommand.reset.rawValue:
            ...
            (() => {
              ...
              if (!host) return JSON.stringify({ ok: false, error: "missing openlocalbotA2UI" });
              return JSON.stringify(host.reset());
            })()
            """)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case openlocalbotCanvasA2UICommand.push.rawValue, openlocalbotCanvasA2UICommand.pushJSONL.rawValue:
            let messages: [AnyCodable]
            if command == openlocalbotCanvasA2UICommand.pushJSONL.rawValue {
                let params = try Self.decodeParams(openlocalbotCanvasA2UIPushJSONLParams.self, from: req.paramsJSON)
                messages = try openlocalbotCanvasA2UIJSONL.decodeMessagesFromJSONL(params.jsonl)
            } else {
                do {
                    let params = try Self.decodeParams(openlocalbotCanvasA2UIPushParams.self, from: req.paramsJSON)
            ...
            let messagesJSON = try openlocalbotCanvasA2UIJSONL.encodeMessagesJSONArray(messages)
            let js = """
            (() => {
              try {
                const host = globalThis.openlocalbotA2UI;
    ...
    private func handleCameraInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        ...
        case openlocalbotCameraCommand.list.rawValue:
            ...
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case openlocalbotCameraCommand.snap.rawValue:
            self.showCameraHUD(text: "Taking photo…", kind: .photo)
            self.triggerCameraFlash()
            let params = (try? Self.decodeParams(openlocalbotCameraSnapParams.self, from: req.paramsJSON)) ??
                openlocalbotCameraSnapParams()
            ...
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case openlocalbotCameraCommand.clip.rawValue:
            let params = (try? Self.decodeParams(openlocalbotCameraClipParams.self, from: req.paramsJSON)) ??
                openlocalbotCameraClipParams()
    ...
    private func handleScreenRecordInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let params = (try? Self.decodeParams(openlocalbotScreenRecordParams.self, from: req.paramsJSON)) ??
            openlocalbotScreenRecordParams()
...
private extension NodeAppModel {
    func locationMode() -> openlocalbotLocationMode {
        let raw = UserDefaults.standard.string(forKey: "location.enabledMode") ?? "off"
        return openlocalbotLocationMode(rawValue: raw) ?? .off
    }
