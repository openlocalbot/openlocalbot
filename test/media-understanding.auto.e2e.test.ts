...
const makeTempMedia = async (ext: string) => {
  const dir = await makeTempDir("openlocalbot-media-e2e-");
...
describe("media understanding auto-detect (e2e)", () => {
  ...
  it("uses sherpa-onnx-offline when available", async () => {
    ...
    try {
      const binDir = await makeTempDir("openlocalbot-bin-sherpa-");
      const modelDir = await makeTempDir("openlocalbot-sherpa-model-");
  ...
  it("uses whisper-cli when sherpa is missing", async () => {
    const snapshot = envSnapshot();
    try {
      const binDir = await makeTempDir("openlocalbot-bin-whispercpp-");
      const modelDir = await makeTempDir("openlocalbot-whispercpp-model-");
      tempPaths.push(binDir, modelDir);
  ...
  it("uses gemini CLI for images when available", async () => {
    const snapshot = envSnapshot();
    try {
      const binDir = await makeTempDir("openlocalbot-bin-gemini-");
