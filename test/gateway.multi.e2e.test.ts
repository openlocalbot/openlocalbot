...
const spawnGatewayInstance = async (name: string): Promise<GatewayInstance> => {
  ...
  const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `openlocalbot-e2e-${name}-`));
  const configDir = path.join(homeDir, ".openlocalbot");
  await fs.mkdir(configDir, { recursive: true });
  const configPath = path.join(configDir, "openlocalbot.json");
    ...
    child = spawn(
      ...
      ],
      {
        ...
        env: {
          ...
          openlocalbot_SKIP_BROWSER_CONTROL_SERVER: "1",
...
const waitForNodeStatus = async (inst: GatewayInstance, nodeId: string, timeoutMs = 10_000) => {
  ...
  while (Date.now() < deadline) {
    const list = (await runCliJson(
      ...
      {
        openlocalbot_GATEWAY_TOKEN: inst.gatewayToken,
        openlocalbot_GATEWAY_PASSWORD: "",
      },
    )) as NodeListPayload;
    ...
    async () => {
      ...
      const [healthA, healthB] = (await Promise.all([
        runCliJson(["health", "--json", "--timeout", "10000"], {
          openlocalbot_GATEWAY_PORT: String(gwA.port),
          openlocalbot_GATEWAY_TOKEN: gwA.gatewayToken,
          openlocalbot_GATEWAY_PASSWORD: "",
        }),
        runCliJson(["health", "--json", "--timeout", "10000"], {
          openlocalbot_GATEWAY_PORT: String(gwB.port),
          openlocalbot_GATEWAY_TOKEN: gwB.gatewayToken,
          openlocalbot_GATEWAY_PASSWORD: "",
        }),
