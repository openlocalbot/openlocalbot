...
function restoreEnv(snapshot: EnvSnapshot) {
  ...
  restoreKey("openlocalbot_STATE_DIR", snapshot.stateDir);
}
...
function setTempHome(base: string) {
  ...
  process.env.openlocalbot_STATE_DIR = path.join(base, ".openlocalbot");
...
export async function withTempHome<T>(
  ...
  const base = await fs.mkdtemp(path.join(os.tmpdir(), opts.prefix ?? "openlocalbot-test-home-"));
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".openlocalbot", "agents", "main", "sessions"), { recursive: true });
  if (opts.env) {
