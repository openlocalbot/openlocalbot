# openlocalbot ACP Bridge

This document describes how the openlocalbot ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`openlocalbot acp` exposes an ACP agent over stdio and forwards prompts to a running
openlocalbot Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
drive a openlocalbot Gateway session.

Quick steps:

1. Run a Gateway (local or remote).
2. Configure the Gateway target (`gateway.remote.url` + auth) or pass flags.
3. Point the IDE to run `openlocalbot acp` over stdio.

Example config:

```bash
openlocalbot config set gateway.remote.url wss://gateway-host:18789
openlocalbot config set gateway.remote.token <token>
```

Example run:

```bash
openlocalbot acp --url wss://gateway-host:18789 --token <token>
```
...
```bash
openlocalbot acp --session agent:main:main
openlocalbot acp --session agent:design:main
openlocalbot acp --session agent:qa:bug-123
```
...
In Zed, open the Agent panel and select “openlocalbot ACP” to start a thread.

## Execution Model

- ACP client spawns `openlocalbot acp` and speaks ACP messages over stdio.
...
```bash
openlocalbot acp --session agent:main:main
openlocalbot acp --session-label "support inbox"
openlocalbot acp --reset-session
```
...
`openlocalbot acp` resolves the Gateway URL and auth from CLI flags or config:
