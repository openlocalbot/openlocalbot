...
- **Ask default:** `on-miss`.
- **Approvals store:** `~/.openlocalbot/exec-approvals.json` (JSON, no legacy migration).
- **Runner:** headless system service; UI app hosts a Unix socket for approvals.
- **Node identity:** use existing `nodeId`.
- **Socket auth:** Unix socket + token (cross-platform); split later if needed.
- **Node host state:** `~/.openlocalbot/node.json` (node id + pairing token).
...
Path: `~/.openlocalbot/exec-approvals.json`
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.openlocalbot/exec-approvals.sock",
...
- Unix socket at `~/.openlocalbot/exec-approvals.sock` (0600).
