...
- Publish as `openlocalbot/plugin-sdk` (or export from core under `openlocalbot/plugin-sdk`).
- Semver with explicit stability guarantees.

### 2) Plugin Runtime (execution surface, injected)

Scope: everything that touches core runtime behavior.
Accessed via `openlocalbotPluginApi.runtime` so plugins never import `src/**`.
...
- Introduce `openlocalbot/plugin-sdk`.
- Add `api.runtime` to `openlocalbotPluginApi` with the surface above.
...
- Plugins declare a required runtime range (e.g., `openlocalbotRuntime: ">=2026.2.0"`).
