---
summary: "Use Synthetic's Anthropic-compatible API in openlocalbot"
read_when:
...
Synthetic exposes Anthropic-compatible endpoints. openlocalbot registers it as the
...
```bash
openlocalbot onboard --auth-choice synthetic-api-key
```
...
Note: openlocalbot's Anthropic client appends `/v1` to the base URL, so use
