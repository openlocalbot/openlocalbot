---
summary: "Use Venice AI privacy-focused models in openlocalbot"
read_when:
  - You want privacy-focused inference in openlocalbot
...
```bash
openlocalbot onboard --auth-choice venice-api-key
```
...
```bash
openlocalbot onboard --non-interactive \
...
```bash
openlocalbot chat --model venice/llama-3.3-70b "Hello, are you working?"
```

## Model Selection

After setup, openlocalbot shows all available Venice models. Pick based on your needs:
...
```bash
openlocalbot models set venice/claude-opus-45
openlocalbot models set venice/llama-3.3-70b
```

List all available models:

```bash
openlocalbot models list | grep venice
```

## Configure via `openlocalbot configure`

1. Run `openlocalbot configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
openlocalbot automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
openlocalbot chat --model venice/llama-3.3-70b

# Use Claude via Venice (anonymized)
openlocalbot chat --model venice/claude-opus-45

# Use uncensored model
openlocalbot chat --model venice/venice-uncensored

# Use vision model with image
openlocalbot chat --model venice/qwen3-vl-235b-a22b

# Use coding model
openlocalbot chat --model venice/qwen3-coder-480b-a35b-instruct
```
...
The Venice model catalog updates dynamically. Run `openlocalbot models list` to see currently available models. Some models may be temporarily offline.
