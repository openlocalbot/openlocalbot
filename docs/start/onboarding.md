---
summary: "First-run onboarding flow for openlocalbot (macOS app)"
read_when:
...
- Writes credentials to `~/.openlocalbot/credentials/oauth.json`
...
The app can install the global `openlocalbot` CLI via npm/pnpm so terminal
...
On the first agent run, openlocalbot bootstraps a workspace (default `~/.openlocalbot/workspace`):
...
```bash
openlocalbot webhooks gmail setup --account you@gmail.com
```
...
- `~/.openlocalbot/credentials/oauth.json`
