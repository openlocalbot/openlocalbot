---
summary: "openlocalbot on Raspberry Pi (budget self-hosted setup)"
read_when:
  - Setting up openlocalbot on a Raspberry Pi
  - Running openlocalbot on ARM devices
  - Building a cheap always-on personal AI
title: "Raspberry Pi"
---

# openlocalbot on Raspberry Pi

## Goal

Run a persistent, always-on openlocalbot Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
Most openlocalbot features work on ARM64, but some external binaries may need ARM builds:
...
sudo systemctl is-enabled openlocalbot

# Enable if not
sudo systemctl enable openlocalbot

# Start on boot
sudo systemctl start openlocalbot
```
...
```bash
# Check logs
journalctl -u openlocalbot --no-pager -n 100

# Common fix: rebuild
cd ~/openlocalbot  # if using hackable install
npm run build
sudo systemctl restart openlocalbot
```
