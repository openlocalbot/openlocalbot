...
service via the external `openlocalbot` CLI (no embedded runtime). This gives you
...
- The app installs a per‑user LaunchAgent labeled `bot.molt.gateway`
  (or `bot.molt.<profile>` when using `--profile`/`openlocalbot_PROFILE`; legacy `com.openlocalbot.*` is supported).
