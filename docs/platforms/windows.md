...
read_when:
  - Installing openlocalbot on Windows
...
openlocalbot on Windows is recommended **via WSL2** (Ubuntu recommended). The
...
- Remote nodes must point at a **reachable** Gateway URL (not `127.0.0.1`); use
  `openlocalbot status --all` to confirm.
...
### 3) Install openlocalbot (inside WSL)
