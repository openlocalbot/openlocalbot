...
This guide summarizes a sane workflow for working on the pi integration in openlocalbot.
...
- Trigger the agent directly:
  - `pnpm openlocalbot agent --message "Hello" --thinking low`
...
State lives under the openlocalbot state directory. Default is `~/.openlocalbot`. If `openlocalbot_STATE_DIR` is set, use that directory instead.

To reset everything:

- `openlocalbot.json` for config
...
- https://docs.openlocalbot.ai/testing
- https://docs.openlocalbot.ai/start/getting-started
