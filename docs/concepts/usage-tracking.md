...
- `/usage cost` in chats: local cost summary aggregated from openlocalbot session logs.
- CLI: `openlocalbot status --usage` prints a full per-provider breakdown.
- CLI: `openlocalbot channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
