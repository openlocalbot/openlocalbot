...
read_when:
  - You want to understand what “context” means in openlocalbot
...
“Context” is **everything openlocalbot sends to the model for a run**. It is bounded by the model’s **context window** (token limit).

Beginner mental model:

- **System prompt** (openlocalbot-built): rules, tools, skills list, time/runtime, and injected workspace files.
...
## How openlocalbot builds the system prompt

The system prompt is **openlocalbot-owned** and rebuilt each run. It includes:
...
By default, openlocalbot injects a fixed set of workspace files (if present):
