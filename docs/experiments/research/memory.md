...
read_when:
  - Designing workspace memory (~/.openlocalbot/workspace) beyond daily Markdown logs
  - Deciding: standalone CLI vs deep openlocalbot integration
...
Target: Clawd-style workspace (`agents.defaults.workspace`, default `~/.openlocalbot/workspace`) where “memory” is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
Keep `~/.openlocalbot/workspace` as canonical human-readable memory.
...
```
~/.openlocalbot/workspace/.memory/index.sqlite
```
...
Recommendation: **deep integration in openlocalbot**, but keep a separable core library.

### Why integrate into openlocalbot?

- openlocalbot already knows:
  - the workspace path (`agents.defaults.workspace`)
  - the session model + heartbeats
  - logging + troubleshooting patterns
- You want the agent itself to call the tools:
  - `openlocalbot memory recall "…" --k 25 --since 30d`
  - `openlocalbot memory reflect --since 7d`
...
Pragmatic take for `~/.openlocalbot/workspace`:
