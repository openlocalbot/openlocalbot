...
openlocalbot features that can generate provider usage or paid API calls.
...
- `openlocalbot status --usage` and `openlocalbot channels list` show provider **usage windows**
...
openlocalbot can pick up credentials from:
...
- `openlocalbot status --usage`
- `openlocalbot models status --json`
...
`openlocalbot models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
