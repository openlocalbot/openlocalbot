...
- Jobs persist under `~/.openlocalbot/cron/` so restarts don’t lose schedules.
...
openlocalbot cron list
openlocalbot cron run <job-id> --force
openlocalbot cron runs --id <job-id>
```
...
Cron jobs are persisted on the Gateway host at `~/.openlocalbot/cron/jobs.json` by default.
The Gateway loads the file into memory and writes it back on changes, so manual edits
are only safe when the Gateway is stopped. Prefer `openlocalbot cron add/edit` or the cron
...
- Job store: `~/.openlocalbot/cron/jobs.json` (Gateway-managed JSON).
- Run history: `~/.openlocalbot/cron/runs/<jobId>.jsonl` (JSONL, auto-pruned).
...
```json5
{
  cron: {
    ...
    store: "~/.openlocalbot/cron/jobs.json",
...
openlocalbot cron add --name "Ops sweep" --cron "0 6 * * *" --session isolated --message "Check ops queue" --agent ops

# Switch or clear the agent on an existing job
openlocalbot cron edit <jobId> --agent ops
openlocalbot cron edit <jobId> --clear-agent
```
...
```bash
openlocalbot system event --mode now --text "Next heartbeat: check battery."
```
...
- `cron.run` (force or due), `cron.runs`
  For immediate system events without a job, use [`openlocalbot system event`](/cli/system).

## Troubleshooting

### “Nothing runs”

- Check cron is enabled: `cron.enabled` and `openlocalbot_SKIP_CRON`.
