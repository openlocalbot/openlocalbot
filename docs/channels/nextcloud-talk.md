...
```bash
openlocalbot plugins install @openlocalbot/nextcloud-talk
```

Local checkout (when running from a git repo):

```bash
openlocalbot plugins install ./extensions/nextcloud-talk
```
...
2. On your Nextcloud server, create a bot:
   ```bash
   ./occ talk:bot:install "openlocalbot" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `openlocalbot pairing list nextcloud-talk`
  - `openlocalbot pairing approve nextcloud-talk <CODE>`
