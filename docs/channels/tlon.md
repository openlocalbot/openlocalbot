...
Tlon is a decentralized messenger built on Urbit. openlocalbot connects to your Urbit ship and can
...
```bash
openlocalbot plugins install @openlocalbot/tlon
```

Local checkout (when running from a git repo):

```bash
openlocalbot plugins install ./extensions/tlon
```
...
Use these with `openlocalbot message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, openlocalbot replies in-thread.
