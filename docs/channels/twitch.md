...
read_when:
  - Setting up Twitch chat integration for openlocalbot
title: "Twitch"
---

# Twitch (plugin)

Twitch chat support via IRC connection. openlocalbot connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
openlocalbot plugins install @openlocalbot/twitch
```

Local checkout (when running from a git repo):

```bash
openlocalbot plugins install ./extensions/twitch
```
...
4. Configure the token:
   - Env: `openlocalbot_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "openlocalbot", // Bot's Twitch account
      accessToken: "oauth:abc123...", // OAuth Access Token (or use openlocalbot_TWITCH_ACCESS_TOKEN env var)
...
```bash
openlocalbot_TWITCH_ACCESS_TOKEN=oauth:abc123...
```
