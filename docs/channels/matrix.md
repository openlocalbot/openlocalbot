...
Matrix is an open, decentralized messaging protocol. openlocalbot connects as a Matrix **user**
...
```bash
openlocalbot plugins install @openlocalbot/matrix
```

Local checkout (when running from a git repo):

```bash
openlocalbot plugins install ./extensions/matrix
```
...
1. Install the Matrix plugin:
   - From npm: `openlocalbot plugins install @openlocalbot/matrix`
   - From a local checkout: `openlocalbot plugins install ./extensions/matrix`
...
3. Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: openlocalbot calls the same
     login endpoint, stores the access token in `~/.openlocalbot/credentials/matrix/credentials.json`,
...
- On first connection, openlocalbot requests device verification from your other sessions.
- Verify the device in another Matrix client (Element, etc.) to enable key sharing.
- If the crypto module cannot be loaded, E2EE is disabled and encrypted rooms will not decrypt;
  openlocalbot logs a warning.
...
`~/.openlocalbot/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`
...
- Approve via:
  - `openlocalbot pairing list matrix`
  - `openlocalbot pairing approve matrix <CODE>`
...
- On startup, openlocalbot resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are kept as typed.
