...
read_when:
  - You want openlocalbot to receive DMs via Nostr
...
Nostr is a decentralized protocol for social networking. This channel enables openlocalbot to receive and respond to encrypted direct messages (DMs) via NIP-04.

## Install (on demand)

### Onboarding (recommended)

- The onboarding wizard (`openlocalbot onboard`) and `openlocalbot channels add` list optional channel plugins.
...
```bash
openlocalbot plugins install @openlocalbot/nostr
```

Use a local checkout (dev workflows):

```bash
openlocalbot plugins install --link <path-to-openlocalbot>/extensions/nostr
```
...
```json
{
  "channels": {
    "nostr": {
      ...
      "profile": {
        "name": "openlocalbot",
        "displayName": "openlocalbot",
        "about": "Personal assistant DM bot",
        "picture": "https://example.com/avatar.png",
        "banner": "https://example.com/banner.png",
        "website": "https://example.com",
        "nip05": "openlocalbot@example.com",
        "lud16": "openlocalbot@example.com"
      }
    }
  }
}
