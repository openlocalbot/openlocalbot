...
```bash
openlocalbot tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `openlocalbot tui --deliver`
...
- Check the Gateway logs: `openlocalbot logs --follow`.
- Confirm the agent can run: `openlocalbot status` and `openlocalbot models status`.
...
- No agents in picker: check `openlocalbot agents list` and your routing config.
