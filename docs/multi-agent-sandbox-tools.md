...
For debugging “why is this blocked?”, see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `openlocalbot sandbox explain`.
...
Legacy `agent.*` configs are migrated by `openlocalbot doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
2. **Verify sandbox containers:**
   ...
   ```exec
   docker ps --filter "name=openlocalbot-sbx-"
   ```
...
4. **Monitor logs:**
   ```exec
   tail -f "${openlocalbot_STATE_DIR:-$HOME/.openlocalbot}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
