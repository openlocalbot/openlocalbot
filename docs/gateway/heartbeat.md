...
- During heartbeat runs, openlocalbot treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `openlocalbot sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, openlocalbot skips the heartbeat run entirely (no model call).
...
```bash
openlocalbot system event --text "Check for urgent follow-ups" --mode now
```
