...
- Replaces the legacy `gateway` command. CLI entry point: `openlocalbot gateway`.
- Runs until stopped; exits non-zero on fatal errors so the supervisor restarts it.

## How to run (local)

```bash
openlocalbot gateway --port 18789
# for full debug/trace logs in stdio:
openlocalbot gateway --port 18789 --verbose
# if the port is busy, terminate listeners then start:
openlocalbot gateway --force
# dev loop (auto-reload on TS changes):
pnpm gateway:watch
```

- Config hot reload watches `~/.openlocalbot/openlocalbot.json` (or `openlocalbot_CONFIG_PATH`).
...
- Starts a Canvas file server by default on `canvasHost.port` (default `18793`), serving `http://<gateway-host>:18793/__openlocalbot__/canvas/` from `~/.openlocalbot/workspace/canvas`. Disable with `canvasHost.enabled=false` or `openlocalbot_SKIP_CANVAS_HOST=1`.
...
- Gateway auth is required by default: set `gateway.auth.token` (or `openlocalbot_GATEWAY_TOKEN`) or `gateway.auth.password`. Clients must send `connect.params.auth.token/password` unless using Tailscale Serve identity.
- The wizard now generates a token by default, even on loopback.
- Port precedence: `--port` > `openlocalbot_GATEWAY_PORT` > `gateway.port` > default `18789`.
...
- macOS: `bot.molt.<profile>` (legacy `com.openlocalbot.*` may still exist)
- Linux: `openlocalbot-gateway-<profile>.service`
- Windows: `openlocalbot Gateway (<profile>)`

Install metadata is embedded in the service config:

- `openlocalbot_SERVICE_MARKER=openlocalbot`
- `openlocalbot_SERVICE_KIND=gateway`
- `openlocalbot_SERVICE_VERSION=<version>`
...
```bash
openlocalbot --dev setup
openlocalbot --dev gateway --allow-unconfigured
# then target the dev instance:
openlocalbot --dev status
openlocalbot --dev health
```

Defaults (can be overridden via env/flags/config):

- `openlocalbot_STATE_DIR=~/.openlocalbot-dev`
- `openlocalbot_CONFIG_PATH=~/.openlocalbot-dev/openlocalbot.json`
- `openlocalbot_GATEWAY_PORT=19001` (Gateway WS + HTTP)
- browser control service port = `19003` (derived: `gateway.port+2`, loopback only)
- `canvasHost.port=19005` (derived: `gateway.port+4`)
- `agents.defaults.workspace` default becomes `~/.openlocalbot/workspace-dev` when you run `setup`/`onboard` under `--dev`.

Derived ports (rules of thumb):

- Base port = `gateway.port` (or `openlocalbot_GATEWAY_PORT` / `--port`)
- browser control service port = base + 2 (loopback only)
- `canvasHost.port = base + 4` (or `openlocalbot_CANVAS_HOST_PORT` / config override)
- Browser profile CDP ports auto-allocate from `browser.controlPort + 9 .. + 108` (persisted per profile).

Checklist per instance:

- unique `gateway.port`
- unique `openlocalbot_CONFIG_PATH`
- unique `openlocalbot_STATE_DIR`
...
```bash
openlocalbot --profile main gateway install
openlocalbot --profile rescue gateway install
```

Example:

```bash
openlocalbot_CONFIG_PATH=~/.openlocalbot/a.json openlocalbot_STATE_DIR=~/.openlocalbot-a openlocalbot gateway --port 19001
openlocalbot_CONFIG_PATH=~/.openlocalbot/b.json openlocalbot_STATE_DIR=~/.openlocalbot-b openlocalbot gateway --port 19002
```
...
- `health` — full health snapshot (same shape as `openlocalbot health --json`).
...
- Use launchd to keep the service alive:
  - Program: path to `openlocalbot`
  - Arguments: `gateway`
  - KeepAlive: true
  - StandardOut/Err: file paths or `syslog`
- On failure, launchd restarts; fatal misconfig should keep exiting so the operator notices.
- LaunchAgents are per-user and require a logged-in session; for headless setups use a custom LaunchDaemon (not shipped).
  - `openlocalbot gateway install` writes `~/Library/LaunchAgents/bot.molt.gateway.plist`
    (or `bot.molt.<profile>.plist`; legacy `com.openlocalbot.*` is cleaned up).
  - `openlocalbot doctor` audits the LaunchAgent config and can update it to current defaults.
...
- If other gateway-like services are detected, the CLI warns unless they are openlocalbot profile services.
  ...
  - Cleanup: `openlocalbot gateway uninstall` (current service) and `openlocalbot doctor` (legacy migrations).
- `gateway install` is a no-op when already installed; use `openlocalbot gateway install --force` to reinstall (profile/env/path changes).

Bundled mac app:

- openlocalbot.app can bundle a Node-based gateway relay and install a per-user LaunchAgent labeled
  `bot.molt.gateway` (or `bot.molt.<profile>`; legacy `com.openlocalbot.*` labels still unload cleanly).
- To stop it cleanly, use `openlocalbot gateway stop` (or `launchctl bootout gui/$UID/bot.molt.gateway`).
- To restart, use `openlocalbot gateway restart` (or `launchctl kickstart -k gui/$UID/bot.molt.gateway`).
  - `launchctl` only works if the LaunchAgent is installed; otherwise use `openlocalbot gateway install` first.
  - Replace the label with `bot.molt.<profile>` when running a named profile.

## Supervision (systemd user unit)

openlocalbot installs a **systemd user service** by default on Linux/WSL2. We
...
`openlocalbot gateway install` writes the user unit. `openlocalbot doctor` audits the
unit and can update it to match the current recommended defaults.

Create `~/.config/systemd/user/openlocalbot-gateway[-<profile>].service`:

```
[Unit]
Description=openlocalbot Gateway (profile: <profile>, v<version>)
After=network-online.target
Wants=network-online.target

[Service]
ExecStart=/usr/local/bin/openlocalbot gateway --port 18789
Restart=always
RestartSec=5
Environment=openlocalbot_GATEWAY_TOKEN=
...
```
systemctl --user enable --now openlocalbot-gateway[-<profile>].service
```

**Alternative (system service)** - for always-on or multi-user servers, you can
install a systemd **system** unit instead of a user unit (no lingering needed).
Create `/etc/systemd/system/openlocalbot-gateway[-<profile>].service` (copy the unit above,
switch `WantedBy=multi-user.target`, set `User=` + `WorkingDirectory=`), then:

```
sudo systemctl daemon-reload
sudo systemctl enable --now openlocalbot-gateway[-<profile>].service
```
...
- `openlocalbot gateway health|status` — request health/status over the Gateway WS.
- `openlocalbot message send --target <num> --message "hi" [--media ...]` — send via Gateway (idempotent for WhatsApp).
- `openlocalbot agent --message "hi" --to <num>` — run an agent turn (waits for final by default).
- `openlocalbot gateway call <method> --params '{"k":"v"}'` — raw method invoker for debugging.
- `openlocalbot gateway stop|restart` — stop/restart the supervised gateway service (launchd/systemd).
- Gateway helper subcommands assume a running gateway on `--url`; they no longer auto-spawn one.

## Migration guidance

- Retire uses of `openlocalbot gateway` and the legacy TCP control port.
