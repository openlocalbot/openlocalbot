...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in openlocalbot
...
Running openlocalbot via Node with `tsx` fails at startup with:

```
[openlocalbot] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `openlocalbot status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch openlocalbot.mjs status
  ```
- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node openlocalbot.mjs status` works on Node 25.
