...
openlocalbot can convert outbound replies into audio using ElevenLabs, OpenAI, or Edge TTS.
It works anywhere openlocalbot can send audio; Telegram gets a round voice-note bubble.
...
Edge TTS does **not** require an API key. If no API keys are found, openlocalbot defaults
...
TTS config lives under `messages.tts` in `openlocalbot.json`.
...
```json5
{
  messages: {
    tts: {
      ...
      maxTextLength: 4000,
      timeoutMs: 30000,
      prefsPath: "~/.openlocalbot/settings/tts.json",
    },
  },
}
```
...
- If `provider` is **unset**, openlocalbot prefers `openai` (if key), then `elevenlabs` (if key),
  otherwise `edge`.
...
`~/.openlocalbot/settings/tts.json`, override with `openlocalbot_TTS_PREFS` or
...
- **Edge TTS**: uses `edge.outputFormat` (default `audio-24khz-48kbitrate-mono-mp3`).
  ...
  - If the configured Edge output format fails, openlocalbot retries with MP3.

OpenAI/ElevenLabs formats are fixed; Telegram expects Opus for voice-note UX.

## Auto-TTS behavior

When enabled, openlocalbot:
...
Discord note: `/tts` is a built-in Discord command, so openlocalbot registers
...
/tts limit 2000
/tts summary off
/tts audio Hello from openlocalbot
```
