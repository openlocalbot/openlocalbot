...
Deploy openlocalbot on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
<a
  href="https://render.com/deploy?repo=https://github.com/openlocalbot/openlocalbot"
  target="_blank"
  rel="noreferrer"
>
  Deploy to Render
...
repository configures everything needed to run openlocalbot:
...
services:
  - type: web
    ...
    envVars:
      ...
      - key: openlocalbot_STATE_DIR
        value: /data/.openlocalbot
      - key: openlocalbot_WORKSPACE_DIR
        value: /data/workspace
      - key: openlocalbot_GATEWAY_TOKEN
        generateValue: true # auto-generates a secure token
    disk:
      name: openlocalbot-data
      mountPath: /data
      sizeGB: 1
```
...
The web dashboard is available at `https://<your-service>.onrender.com/openlocalbot`.
...
If you use the original openlocalbot repository, Render will not auto-deploy your openlocalbot. To update it, run a manual Blueprint sync from the dashboard.
...
For openlocalbot, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
This downloads a portable backup you can restore on any openlocalbot host.
