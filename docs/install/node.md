...
read_when:
  - "You installed openlocalbot but `openlocalbot` is “command not found”"
...
openlocalbot’s runtime baseline is **Node 22+**.

If you can run `npm install -g openlocalbot@latest` but later see `openlocalbot: command not found`, it’s almost always a **PATH** issue: the directory where npm puts global binaries isn’t on your shell’s PATH.
