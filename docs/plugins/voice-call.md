...
read_when:
  - You want to place an outbound voice call from openlocalbot
...
Voice calls for openlocalbot via a plugin. Supports outbound notifications and
...
- Use `openlocalbot voicecall ...` or the `voice_call` tool
...
```bash
openlocalbot plugins install ./extensions/voice-call
