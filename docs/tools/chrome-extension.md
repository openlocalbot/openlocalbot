---
summary: "Chrome extension: let openlocalbot drive your existing Chrome tab"
read_when:
...
The openlocalbot Chrome extension lets the agent control your **existing Chrome tabs** (your normal Chrome window) instead of launching a separate openlocalbot-managed Chrome profile.
...
openlocalbot then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
The extension ships inside the openlocalbot release (npm package) as static files. There is no separate “build” step.

After upgrading openlocalbot:

- Re-run `openlocalbot browser extension install` to refresh the installed files under your openlocalbot state directory.
- Chrome → `chrome://extensions` → click “Reload” on the extension.

## Use it (no extra config)

openlocalbot ships with a built-in browser profile named `chrome` that targets the extension relay on the default port.
...
- Open the tab you want openlocalbot to control.
...
- `ON`: attached; openlocalbot can drive that tab.
...
Debugging: `openlocalbot sandbox explain`
...
`openlocalbot browser extension path` prints the **installed** on-disk directory containing the extension files.

The CLI intentionally does **not** print a `node_modules` path. Always run `openlocalbot browser extension install` first to copy the extension to a stable location under your openlocalbot state directory.
...
- **This is not isolated** like the dedicated openlocalbot-managed profile.
