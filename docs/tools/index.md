---
summary: "Agent tool surface for openlocalbot (browser, canvas, nodes, message, cron) replacing legacy `openlocalbot-*` skills"
read_when:
  - Adding or modifying agent tools
  - Retiring or changing `openlocalbot-*` skills
title: "Tools"
---

# Tools (openlocalbot)

openlocalbot exposes **first-class agent tools** for browser, canvas, nodes, and cron.
These replace the old `openlocalbot-*` skills: the tools are typed, no shelling,
and the agent should rely on them directly.

## Disabling tools

You can globally allow/deny tools via `tools.allow` / `tools.deny` in `openlocalbot.json`
...
- If `tools.allow` only references unknown or unloaded plugin tool names, openlocalbot logs a warning and ignores the allowlist so core tools stay available.
...
- `group:nodes`: `nodes`
- `group:openlocalbot`: all built-in openlocalbot tools (excludes provider plugins)
...
- `host=node` can target a macOS companion app or a headless node host (`openlocalbot node run`).
...
- Requires a Brave API key (recommended: `openlocalbot configure --section web`, or set `BRAVE_API_KEY`).
...
Control the dedicated openlocalbot-managed browser.
...
- Quick smoke: `openlocalbot nodes canvas a2ui push --node <id> --text "Hello from A2UI"`.
...
- `restart` (authorizes + sends `SIGUSR1` for in-process restart; `openlocalbot gateway` restart in-place)
