...
import type { openlocalbotPluginApi, openlocalbotPluginToolContext } from "../../../src/plugins/types.js";
import { createLobsterTool } from "./lobster-tool.js";

async function writeFakeLobsterScript(scriptBody: string, prefix = "openlocalbot-lobster-plugin-") {
...
function fakeApi(overrides: Partial<openlocalbotPluginApi> = {}): openlocalbotPluginApi {
  return {
    id: "lobster",
    name: "lobster",
    source: "test",
...
function fakeCtx(overrides: Partial<openlocalbotPluginToolContext> = {}): openlocalbotPluginToolContext {
  return {
    config: {} as any,
    workspaceDir: "/tmp",
    agentDir: "/tmp",
...
describe("lobster plugin tool", () => {
  ...
  it("tolerates noisy stdout before the JSON envelope", async () => {
    ...
    const { dir } = await writeFakeLobsterScript(
      `const payload = ${JSON.stringify(payload)};\n` +
        ...
        `process.stdout.write(JSON.stringify(payload));\n`,
      "openlocalbot-lobster-plugin-noisy-",
    );
  ...
  it("rejects invalid JSON from lobster", async () => {
    const { dir } = await writeFakeLobsterScript(
      ...
      "openlocalbot-lobster-plugin-bad-",
    );
  ...
  it("can be gated off in sandboxed contexts", async () => {
    const api = fakeApi();
    const factoryTool = (ctx: openlocalbotPluginToolContext) => {
      if (ctx.sandboxed) {
        return null;
      }
