...
// NOTE: This extension is intended to be bundled with openlocalbot.
// When running from source (tests/dev), openlocalbot internals live under src/.
...
export function createLlmTaskTool(api: openlocalbotPluginApi) {
  return {
    name: "llm-task",
    description:
      "Run a generic JSON-only LLM task and return schema-validated JSON. Designed for orchestration from Lobster workflows via openlocalbot.invoke.",
    ...
    async execute(_id: string, params: Record<string, unknown>) {
      ...
      try {
        tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "openlocalbot-llm-task-"));
