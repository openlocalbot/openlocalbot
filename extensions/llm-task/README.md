...
`openlocalbot.invoke --each`) without adding new openlocalbot code per workflow.
...
This extension depends on openlocalbot internal modules (the embedded agent runner).
It is intended to ship as a **bundled** openlocalbot extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.openlocalbot/extensions` as a standalone plugin directory.
