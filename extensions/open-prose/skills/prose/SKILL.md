...
metadata: { "openlocalbot": { "emoji": "🪶", "homepage": "https://www.prose.md" } }
---
...
- **Task tool** in the upstream spec == openlocalbot `sessions_spawn`
- **File I/O** == openlocalbot `read`/`write`
- **Remote fetch** == openlocalbot `web_fetch` (or `exec` with curl when POST is required)
