...
function resolveopenlocalbotRoot(): string {
  if (coreRootCache) {
    return coreRootCache;
  }
  const override = process.env.openlocalbot_ROOT?.trim();
  if (override) {
  ...
  for (const start of candidates) {
    for (const name of ["openlocalbot"]) {
  ...
  throw new Error("Unable to resolve core root. Set openlocalbot_ROOT to the package root.");
}

async function importCoreModule<T>(relativePath: string): Promise<T> {
  const root = resolveopenlocalbotRoot();
