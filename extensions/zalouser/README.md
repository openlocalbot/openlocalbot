...
openlocalbot extension for Zalo Personal Account messaging via [zca-cli](https://zca-cli.dev).
...
```bash
openlocalbot channels login --channel zalouser
...
```bash
openlocalbot message send --channel zalouser --target <threadId> --message "Hello from openlocalbot!"
```
...
```bash
openlocalbot channels login --channel zalouser              # Login via QR
openlocalbot channels login --channel zalouser --account work
openlocalbot channels status --probe
openlocalbot channels logout --channel zalouser
```

### Directory (IDs, contacts, groups)

```bash
openlocalbot directory self --channel zalouser
openlocalbot directory peers list --channel zalouser --query "name"
openlocalbot directory groups list --channel zalouser --query "work"
openlocalbot directory groups members --channel zalouser --group-id <id>
```
...
```bash
# Text
openlocalbot message send --channel zalouser --target <threadId> --message "message"

# Media (URL)
openlocalbot message send --channel zalouser --target <threadId> --message "caption" --media-url "https://example.com/img.jpg"
```

### Listener

The listener runs inside the Gateway when the channel is enabled. For debugging,
use `openlocalbot channels logs --channel zalouser` or run `zca listen` directly.
...
```bash
openlocalbot channels login --channel zalouser --account work
openlocalbot message send --channel zalouser --account work --target <id> --message "Hello"
