import type { PluginRuntime } from "openlocalbot/plugin-sdk";
...
async function withTempStateDir<T>(fn: (dir: string) => Promise<T>) {
  const previous = process.env.openlocalbot_STATE_DIR;
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "openlocalbot-nostr-"));
  process.env.openlocalbot_STATE_DIR = dir;
  setNostrRuntime({
    state: {
      resolveStateDir: (env, homedir) => {
        const override = env.openlocalbot_STATE_DIR?.trim() || env.openlocalbot_STATE_DIR?.trim();
        if (override) {
          return override;
        }
        return path.join(homedir(), ".openlocalbot");
      },
    },
  } as PluginRuntime);
  try {
    ...
    if (previous === undefined) {
      ...
      process.env.openlocalbot_STATE_DIR = previous;
    }
