...
export function connectGateway(host: GatewayHost) {
  ...
  host.client = new GatewayBrowserClient({
    ...
    clientName: "openlocalbot-control-ui",
    mode: "webchat",
    onHello: (hello) => {
      host.connected = true;
      host.lastError = null;
      ...
      void loadAssistantIdentity(host as unknown as openlocalbotApp);
      void loadAgents(host as unknown as openlocalbotApp);
      void loadNodes(host as unknown as openlocalbotApp, { quiet: true });
      void loadDevices(host as unknown as openlocalbotApp, { quiet: true });
...
function handleGatewayEventUnsafe(host: GatewayHost, evt: GatewayEventFrame) {
  ...
  if (evt.event === "chat") {
    ...
    const state = handleChatEvent(host as unknown as openlocalbotApp, payload);
    if (state === "final" || state === "error" || state === "aborted") {
      ...
      if (runId && host.refreshSessionsAfterChat.has(runId)) {
        ...
        if (state === "final") {
          void loadSessions(host as unknown as openlocalbotApp, {
            activeMinutes: CHAT_SESSIONS_ACTIVE_MINUTES,
          });
        }
      }
    }
    if (state === "final") void loadChatHistory(host as unknown as openlocalbotApp);
    return;
  }
