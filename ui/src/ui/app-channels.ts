...
export async function handleWhatsAppStart(host: openlocalbotApp, force: boolean) {
  await startWhatsAppLogin(host, force);
  await loadChannels(host, true);
}

export async function handleWhatsAppWait(host: openlocalbotApp) {
  await waitWhatsAppLogin(host);
  await loadChannels(host, true);
}

export async function handleWhatsAppLogout(host: openlocalbotApp) {
  await logoutWhatsApp(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigSave(host: openlocalbotApp) {
  await saveConfig(host);
  await loadConfig(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigReload(host: openlocalbotApp) {
  await loadConfig(host);
  await loadChannels(host, true);
}
...
function resolveNostrAccountId(host: openlocalbotApp): string {
...
export function handleNostrProfileCancel(host: openlocalbotApp) {
...
export function handleNostrProfileToggleAdvanced(host: openlocalbotApp) {
  const state = host.nostrProfileFormState;
  if (!state) return;
  host.nostrProfileFormState = {
    ...state,
...
export async function handleNostrProfileSave(host: openlocalbotApp) {
  const state = host.nostrProfileFormState;
...
export async function handleNostrProfileImport(host: openlocalbotApp) {
  const state = host.nostrProfileFormState;
