...
import { openlocalbotApp } from "./app";
import "../styles.css";

const originalConnect = openlocalbotApp.prototype.connect;

function mountApp(pathname: string) {
  window.history.replaceState({}, "", pathname);
  const app = document.createElement("openlocalbot-app") as openlocalbotApp;
...
beforeEach(() => {
  openlocalbotApp.prototype.connect = () => {
    // no-op: avoid real gateway WS connections in browser tests
  };
  window.__openlocalbot_CONTROL_UI_BASE_PATH__ = undefined;
  localStorage.clear();
  document.body.innerHTML = "";
});

afterEach(() => {
  openlocalbotApp.prototype.connect = originalConnect;
  window.__openlocalbot_CONTROL_UI_BASE_PATH__ = undefined;
  localStorage.clear();
  document.body.innerHTML = "";
});
...
describe("control UI routing", () => {
  ...
  it("infers nested base paths", async () => {
    const app = mountApp("/apps/openlocalbot/cron");
    await app.updateComplete;

    expect(app.basePath).toBe("/apps/openlocalbot");
    expect(app.tab).toBe("cron");
    expect(window.location.pathname).toBe("/apps/openlocalbot/cron");
  });

  it("honors explicit base path overrides", async () => {
    window.__openlocalbot_CONTROL_UI_BASE_PATH__ = "/openlocalbot";
    const app = mountApp("/openlocalbot/sessions");
    await app.updateComplete;

    expect(app.basePath).toBe("/openlocalbot");
    expect(app.tab).toBe("sessions");
    expect(window.location.pathname).toBe("/openlocalbot/sessions");
  });
  ...
  it("hydrates token from URL params even when settings already set", async () => {
    localStorage.setItem(
      "openlocalbot.control.settings.v1",
