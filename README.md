...
<p align="center">
  ...
  <a href="https://www.npmjs.com/package/openlocalbot"><img src="https://img.shields.io/npm/v/openlocalbot?style=for-the-badge" alt="npm version"></a>
</p>

**OpenLocalBot** is a _personal AI assistant_ you run on your own devices.
It answers you on the channels you already use (WhatsApp, Telegram, Slack, Discord, Google Chat, Signal, iMessage, Microsoft Teams, WebChat), plus extension channels like BlueBubbles, Matrix, Zalo, and Zalo Personal. It can speak and listen on macOS/iOS/Android, and can render a live Canvas you control. The Gateway is just the control plane — the product is the assistant.

If you want a personal, single-user assistant that feels local, fast, and always-on, this is it.

Preferred setup: run the onboarding wizard (`openlocalbot onboard`). It walks through gateway, workspace, channels, and skills. The CLI wizard is the recommended path and works on **macOS, Linux, and Windows (via WSL2; strongly recommended)**.

**New: Web-based onboarding UI** - For a graphical setup experience, use `openlocalbot onboard --web`. This launches a bilingual (English/中文) web interface on port 9887.

Works with npm, pnpm, or bun.
New install? Start here: [Getting started](https://docs.openlocalbot.ai/start/getting-started)
...
- **[DeepSeek](https://platform.deepseek.com/)** — Official DeepSeek V3.2 API (Chat + Reasoner modes)
- **[Groq](https://console.groq.com/)** — Ultra-fast inference (Llama 4, Kimi K2, Qwen 3, GPT-OSS)
- **[OpenCode Zen](https://opencode.ai/)** — Multi-model proxy (Claude, GPT, Gemini, GLM, Kimi)
- **[SiliconFlow 硅基流动](https://cloud.siliconflow.cn/)** — DeepSeek, GLM, Qwen, Llama
- **[NVIDIA NIM](https://build.nvidia.com/)** — Llama, DeepSeek, Qwen, Mistral

Model note: while any model is supported, I strongly recommend **Anthropic Pro/Max (100/200) + Opus 4.5** for long‑context strength and better prompt‑injection resistance. See [Onboarding](https://docs.openlocalbot.ai/start/onboarding).

## Models (selection + auth)

- Models config + CLI: [Models](https://docs.openlocalbot.ai/concepts/models)
- Auth profile rotation (OAuth vs API keys) + fallbacks: [Model failover](https://docs.openlocalbot.ai/concepts/model-failover)
...
# or: pnpm add -g openlocalbot@latest
...
- Model provider selection (Anthropic, OpenAI, Google, SiliconFlow, OpenCode Zen, DeepSeek, Groq, etc.)
...
Full beginner guide (auth, pairing, channels): [Getting started](https://docs.openlocalbot.ai/start/getting-started)
...
# Send a message
openlocalbot message send --to +1234567890 --message "Hello from OpenLocalBot"

# Talk to the assistant (optionally deliver back to any connected channel: WhatsApp/Telegram/Slack/Discord/Google Chat/Signal/iMessage/BlueBubbles/Microsoft Teams/Matrix/Zalo/Zalo Personal/WebChat)
openlocalbot agent --message "Ship checklist" --thinking high
```

Upgrading? [Updating guide](https://docs.openlocalbot.ai/install/updating) (and run `openlocalbot doctor`).
...
Switch channels (git + npm): `openlocalbot update --channel stable|beta|dev`.
Details: [Development channels](https://docs.openlocalbot.ai/install/development-channels).
...
Note: `pnpm openlocalbot ...` runs TypeScript directly (via `tsx`). `pnpm build` produces `dist/` for running via Node / the packaged `openlocalbot` binary.

## Security defaults (DM access)

OpenLocalBot connects to real messaging surfaces. Treat inbound DMs as **untrusted input**.

Full security guide: [Security](https://docs.openlocalbot.ai/gateway/security)

Default behavior on Telegram/WhatsApp/Signal/iMessage/Microsoft Teams/Discord/Google Chat/Slack:

- **DM pairing** (`dmPolicy="pairing"` / `channels.discord.dm.policy="pairing"` / `channels.slack.dm.policy="pairing"`): unknown senders receive a short pairing code and the bot does not process their message.
- Approve with: `openlocalbot pairing approve <channel> <code>` (then the sender is added to a local allowlist store).
- Public inbound DMs require an explicit opt-in: set `dmPolicy="open"` and include `"*"` in the channel allowlist (`allowFrom` / `channels.discord.dm.allowFrom` / `channels.slack.dm.allowFrom`).

Run `openlocalbot doctor` to surface risky/misconfigured DM policies.

## Highlights

- **[Local-first Gateway](https://docs.openlocalbot.ai/gateway)** — single control plane for sessions, channels, tools, and events.
- **[Multi-channel inbox](https://docs.openlocalbot.ai/channels)** — WhatsApp, Telegram, Slack, Discord, Google Chat, Signal, iMessage, BlueBubbles, Microsoft Teams, Matrix, Zalo, Zalo Personal, WebChat, macOS, iOS/Android.
- **[Multi-agent routing](https://docs.openlocalbot.ai/gateway/configuration)** — route inbound channels/accounts/peers to isolated agents (workspaces + per-agent sessions).
- **[Voice Wake](https://docs.openlocalbot.ai/nodes/voicewake) + [Talk Mode](https://docs.openlocalbot.ai/nodes/talk)** — always-on speech for macOS/iOS/Android with ElevenLabs.
- **[Live Canvas](https://docs.openlocalbot.ai/platforms/mac/canvas)** — agent-driven visual workspace with [A2UI](https://docs.openlocalbot.ai/platforms/mac/canvas#canvas-a2ui).
- **[First-class tools](https://docs.openlocalbot.ai/tools)** — browser, canvas, nodes, cron, sessions, and Discord/Slack actions.
- **[Companion apps](https://docs.openlocalbot.ai/platforms/macos)** — macOS menu bar app + iOS/Android [nodes](https://docs.openlocalbot.ai/nodes).
- **[Onboarding](https://docs.openlocalbot.ai/start/wizard) + [skills](https://docs.openlocalbot.ai/tools/skills)** — wizard-driven setup with bundled/managed/workspace skills.
...
- [Gateway WS control plane](https://docs.openlocalbot.ai/gateway) with sessions, presence, config, cron, webhooks, [Control UI](https://docs.openlocalbot.ai/web), and [Canvas host](https://docs.openlocalbot.ai/platforms/mac/canvas#canvas-a2ui).
- [CLI surface](https://docs.openlocalbot.ai/tools/agent-send): gateway, agent, send, [wizard](https://docs.openlocalbot.ai/start/wizard), and [doctor](https://docs.openlocalbot.ai/gateway/doctor).
- [Pi agent runtime](https://docs.openlocalbot.ai/concepts/agent) in RPC mode with tool streaming and block streaming.
- [Session model](https://docs.openlocalbot.ai/concepts/session): `main` for direct chats, group isolation, activation modes, queue modes, reply-back. Group rules: [Groups](https://docs.openlocalbot.ai/concepts/groups).
- [Media pipeline](https://docs.openlocalbot.ai/nodes/images): images/audio/video, transcription hooks, size caps, temp file lifecycle. Audio details: [Audio](https://docs.openlocalbot.ai/nodes/audio).

### Channels

- [Channels](https://docs.openlocalbot.ai/channels): [WhatsApp](https://docs.openlocalbot.ai/channels/whatsapp) (Baileys), [Telegram](https://docs.openlocalbot.ai/channels/telegram) (grammY), [Slack](https://docs.openlocalbot.ai/channels/slack) (Bolt), [Discord](https://docs.openlocalbot.ai/channels/discord) (discord.js), [Google Chat](https://docs.openlocalbot.ai/channels/googlechat) (Chat API), [Signal](https://docs.openlocalbot.ai/channels/signal) (signal-cli), [iMessage](https://docs.openlocalbot.ai/channels/imessage) (imsg), [BlueBubbles](https://docs.openlocalbot.ai/channels/bluebubbles) (extension), [Microsoft Teams](https://docs.openlocalbot.ai/channels/msteams) (extension), [Matrix](https://docs.openlocalbot.ai/channels/matrix) (extension), [Zalo](https://docs.openlocalbot.ai/channels/zalo) (extension), [Zalo Personal](https://docs.openlocalbot.ai/channels/zalouser) (extension), [WebChat](https://docs.openlocalbot.ai/web/webchat).
- [Group routing](https://docs.openlocalbot.ai/concepts/group-messages): mention gating, reply tags, per-channel chunking and routing. Channel rules: [Channels](https://docs.openlocalbot.ai/channels).
...
- [iOS node](https://docs.openlocalbot.ai/platforms/ios): [Canvas](https://docs.openlocalbot.ai/platforms/mac/canvas), [Voice Wake](https://docs.openlocalbot.ai/nodes/voicewake), [Talk Mode](https://docs.openlocalbot.ai/nodes/talk), camera, screen recording, Bonjour pairing.
- [Android node](https://docs.openlocalbot.ai/platforms/android): [Canvas](https://docs.openlocalbot.ai/platforms/mac/canvas), [Talk Mode](https://docs.openlocalbot.ai/nodes/talk), camera, screen recording, optional SMS.
- [macOS node mode](https://docs.openlocalbot.ai/nodes): system.run/notify + canvas/camera exposure.

### Tools + automation

- [Browser control](https://docs.openlocalbot.ai/tools/browser): dedicated openlocalbot Chrome/Chromium, snapshots, actions, uploads, profiles.
- [Canvas](https://docs.openlocalbot.ai/platforms/mac/canvas): [A2UI](https://docs.openlocalbot.ai/platforms/mac/canvas#canvas-a2ui) push/reset, eval, snapshot.
- [Nodes](https://docs.openlocalbot.ai/nodes): camera snap/clip, screen record, [location.get](https://docs.openlocalbot.ai/nodes/location-command), notifications.
- [Cron + wakeups](https://docs.openlocalbot.ai/automation/cron-jobs); [webhooks](https://docs.openlocalbot.ai/automation/webhook); [Gmail Pub/Sub](https://docs.openlocalbot.ai/automation/gmail-pubsub).
- [Skills platform](https://docs.openlocalbot.ai/tools/skills): bundled, managed, and workspace skills with install gating + UI.

### Runtime + safety

- [Channel routing](https://docs.openlocalbot.ai/concepts/channel-routing), [retry policy](https://docs.openlocalbot.ai/concepts/retry), and [streaming/chunking](https://docs.openlocalbot.ai/concepts/streaming).
- [Presence](https://docs.openlocalbot.ai/concepts/presence), [typing indicators](https://docs.openlocalbot.ai/concepts/typing-indicators), and [usage tracking](https://docs.openlocalbot.ai/concepts/usage-tracking).
- [Models](https://docs.openlocalbot.ai/concepts/models), [model failover](https://docs.openlocalbot.ai/concepts/model-failover), and [session pruning](https://docs.openlocalbot.ai/concepts/session-pruning).
- [Security](https://docs.openlocalbot.ai/gateway/security) and [troubleshooting](https://docs.openlocalbot.ai/channels/troubleshooting).

### Ops + packaging

- [Control UI](https://docs.openlocalbot.ai/web) + [WebChat](https://docs.openlocalbot.ai/web/webchat) served directly from the Gateway.
- [Tailscale Serve/Funnel](https://docs.openlocalbot.ai/gateway/tailscale) or [SSH tunnels](https://docs.openlocalbot.ai/gateway/remote) with token/password auth.
- [Nix mode](https://docs.openlocalbot.ai/install/nix) for declarative config; [Docker](https://docs.openlocalbot.ai/install/docker)-based installs.
- [Doctor](https://docs.openlocalbot.ai/gateway/doctor) migrations, [logging](https://docs.openlocalbot.ai/logging).
...
- **[Gateway WebSocket network](https://docs.openlocalbot.ai/concepts/architecture)** — single WS control plane for clients, tools, and events (plus ops: [Gateway runbook](https://docs.openlocalbot.ai/gateway)).
- **[Tailscale exposure](https://docs.openlocalbot.ai/gateway/tailscale)** — Serve/Funnel for the Gateway dashboard + WS (remote access: [Remote](https://docs.openlocalbot.ai/gateway/remote)).
- **[Browser control](https://docs.openlocalbot.ai/tools/browser)** — openlocalbot‑managed Chrome/Chromium with CDP control.
- **[Canvas + A2UI](https://docs.openlocalbot.ai/platforms/mac/canvas)** — agent‑driven visual workspace (A2UI host: [Canvas/A2UI](https://docs.openlocalbot.ai/platforms/mac/canvas#canvas-a2ui)).
- **[Voice Wake](https://docs.openlocalbot.ai/nodes/voicewake) + [Talk Mode](https://docs.openlocalbot.ai/nodes/talk)** — always‑on speech and continuous conversation.
- **[Nodes](https://docs.openlocalbot.ai/nodes)** — Canvas, camera snap/clip, screen record, `location.get`, notifications, plus macOS‑only `system.run`/`system.notify`.

## Tailscale access (Gateway dashboard)

OpenLocalBot can auto-configure Tailscale **Serve** (tailnet-only) or **Funnel** (public) while the Gateway stays bound to loopback. Configure `gateway.tailscale.mode`:
...
- `gateway.bind` must stay `loopback` when Serve/Funnel is enabled (OpenLocalBot enforces this).
- Serve can be forced to require a password by setting `gateway.auth.mode: "password"` or `gateway.auth.allowTailscale: false`.
- Funnel refuses to start unless `gateway.auth.mode: "password"` is set.
- Optional: `gateway.tailscale.resetOnExit` to undo Serve/Funnel on shutdown.

Details: [Tailscale guide](https://docs.openlocalbot.ai/gateway/tailscale) · [Web surfaces](https://docs.openlocalbot.ai/web)
...
Details: [Remote access](https://docs.openlocalbot.ai/gateway/remote) · [Nodes](https://docs.openlocalbot.ai/nodes) · [Security](https://docs.openlocalbot.ai/gateway/security)
...
Details: [Nodes](https://docs.openlocalbot.ai/nodes) · [macOS app](https://docs.openlocalbot.ai/platforms/macos) · [Gateway protocol](https://docs.openlocalbot.ai/concepts/architecture)
...
Details: [Session tools](https://docs.openlocalbot.ai/concepts/session-tool)
...
### macOS (OpenLocalBot.app) (optional)
...
- Controlled via `openlocalbot nodes …`.

Runbook: [iOS connect](https://docs.openlocalbot.ai/platforms/ios).

### Android node (optional)

- Pairs via the same Bridge + pairing flow as iOS.
- Exposes Canvas, Camera, and Screen capture commands.
- Runbook: [Android connect](https://docs.openlocalbot.ai/platforms/android).

## Agent workspace + skills

- Workspace root: `~/.openlocalbot/workspace` (configurable via `agents.defaults.workspace`).
- Injected prompt files: `AGENTS.md`, `SOUL.md`, `TOOLS.md`.
- Skills: `~/.openlocalbot/workspace/skills/<skill>/SKILL.md`.

## Configuration

Minimal `~/.openlocalbot/openlocalbot.json` (model + defaults):
...
[Full configuration reference (all keys + examples).](https://docs.openlocalbot.ai/gateway/configuration)
...
Details: [Security guide](https://docs.openlocalbot.ai/gateway/security) · [Docker + sandboxing](https://docs.openlocalbot.ai/install/docker) · [Sandbox config](https://docs.openlocalbot.ai/gateway/configuration)

### [WhatsApp](https://docs.openlocalbot.ai/channels/whatsapp)

- Link the device: `pnpm openlocalbot channels login` (stores creds in `~/.openlocalbot/credentials`).
...
- [Start with the docs index for navigation and “what’s where.”](https://docs.openlocalbot.ai)
- [Read the architecture overview for the gateway + protocol model.](https://docs.openlocalbot.ai/concepts/architecture)
- [Use the full configuration reference when you need every key and example.](https://docs.openlocalbot.ai/gateway/configuration)
- [Run the Gateway by the book with the operational runbook.](https://docs.openlocalbot.ai/gateway)
- [Learn how the Control UI/Web surfaces work and how to expose them safely.](https://docs.openlocalbot.ai/web)
- [Understand remote access over SSH tunnels or tailnets.](https://docs.openlocalbot.ai/gateway/remote)
- [Follow the onboarding wizard flow for a guided setup.](https://docs.openlocalbot.ai/start/wizard)
- [Wire external triggers via the webhook surface.](https://docs.openlocalbot.ai/automation/webhook)
- [Set up Gmail Pub/Sub triggers.](https://docs.openlocalbot.ai/automation/gmail-pubsub)
- [Learn the macOS menu bar companion details.](https://docs.openlocalbot.ai/platforms/mac/menu-bar)
- [Platform guides: Windows (WSL2)](https://docs.openlocalbot.ai/platforms/windows), [Linux](https://docs.openlocalbot.ai/platforms/linux), [macOS](https://docs.openlocalbot.ai/platforms/macos), [iOS](https://docs.openlocalbot.ai/platforms/ios), [Android](https://docs.openlocalbot.ai/platforms/android)
- [Debug common failures with the troubleshooting guide.](https://docs.openlocalbot.ai/channels/troubleshooting)
- [Review security guidance before exposing anything.](https://docs.openlocalbot.ai/gateway/security)

## Advanced docs (discovery + control)

- [Discovery + transports](https://docs.openlocalbot.ai/gateway/discovery)
- [Bonjour/mDNS](https://docs.openlocalbot.ai/gateway/bonjour)
- [Gateway pairing](https://docs.openlocalbot.ai/gateway/pairing)
- [Remote gateway README](https://docs.openlocalbot.ai/gateway/remote-gateway-readme)
- [Control UI](https://docs.openlocalbot.ai/web/control-ui)
- [Dashboard](https://docs.openlocalbot.ai/web/dashboard)

## Operations & troubleshooting

- [Health checks](https://docs.openlocalbot.ai/gateway/health)
- [Gateway lock](https://docs.openlocalbot.ai/gateway/gateway-lock)
- [Background process](https://docs.openlocalbot.ai/gateway/background-process)
- [Browser troubleshooting (Linux)](https://docs.openlocalbot.ai/tools/browser-linux-troubleshooting)
- [Logging](https://docs.openlocalbot.ai/logging)

## Deep dives

- [Agent loop](https://docs.openlocalbot.ai/concepts/agent-loop)
- [Presence](https://docs.openlocalbot.ai/concepts/presence)
- [TypeBox schemas](https://docs.openlocalbot.ai/concepts/typebox)
- [RPC adapters](https://docs.openlocalbot.ai/reference/rpc)
- [Queue](https://docs.openlocalbot.ai/concepts/queue)

## Workspace & skills

- [Skills config](https://docs.openlocalbot.ai/tools/skills-config)
- [Default AGENTS](https://docs.openlocalbot.ai/reference/AGENTS.default)
- [Templates: AGENTS](https://docs.openlocalbot.ai/reference/templates/AGENTS)
- [Templates: BOOTSTRAP](https://docs.openlocalbot.ai/reference/templates/BOOTSTRAP)
- [Templates: IDENTITY](https://docs.openlocalbot.ai/reference/templates/IDENTITY)
- [Templates: SOUL](https://docs.openlocalbot.ai/reference/templates/SOUL)
- [Templates: TOOLS](https://docs.openlocalbot.ai/reference/templates/TOOLS)
- [Templates: USER](https://docs.openlocalbot.ai/reference/templates/USER)

## Platform internals

- [macOS dev setup](https://docs.openlocalbot.ai/platforms/mac/dev-setup)
- [macOS menu bar](https://docs.openlocalbot.ai/platforms/mac/menu-bar)
- [macOS voice wake](https://docs.openlocalbot.ai/platforms/mac/voicewake)
- [iOS node](https://docs.openlocalbot.ai/platforms/ios)
- [Android node](https://docs.openlocalbot.ai/platforms/android)
- [Windows (WSL2)](https://docs.openlocalbot.ai/platforms/windows)
- [Linux app](https://docs.openlocalbot.ai/platforms/linux)
...
OpenLocalBot was built for **Molty**, a space lobster AI assistant. 🦞
